...
export function validateCoachReportPersistentHistoryCopy(): readonly string[] {
  ...
  for (const term of forbiddenTerms) {
    assertTest(!visible.includes(term), `visible persistent-history copy must not contain ${term}.`);
  }
...
if (require.main === module) {
  const checks = validateCoachReportPersistentHistoryCopy();
  console.log("coachReportPersistentHistoryCopy tests passed.");
