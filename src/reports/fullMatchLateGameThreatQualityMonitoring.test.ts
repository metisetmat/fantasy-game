...
assert.ok(model.lateGameThreatAutomaticityAudit.lateGameThreatQualityRate <= 100);
assert.ok(model.lateGameThreatAutomaticityAudit.lateGamePressureToThreatRate <= 100);
assert.equal(model.trailingTeamScoreChangeInjected, model.trailingTeamScoreChangeInjectedCountAfter > 0);
assert.equal(model.trailingTeamScoringEventInjected, model.trailingTeamScoringEventInjectedCountAfter > 0);
assert.equal(model.trailingTeamOpportunityForced, model.trailingTeamOpportunityForcedCountAfter > 0);
assert.equal(model.forcedTrailingTeamScoreApplied, model.trailingTeamScoreChangeInjectedCountAfter > 0);
