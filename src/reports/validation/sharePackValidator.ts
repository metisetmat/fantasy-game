...
export function validateSharePack(input: { readonly reportDirectory: string }): SharePackValidationResult {
  ...
  const fullMatchWorkbenchChainReplay4V = readIfExists(join(shareDirectory, "fullmatch-workbench-chain-replay-4v.md"));
  const fullMatchWorkbenchChainReplay4VValidation = readIfExists(join(shareDirectory, "validation.fullmatch-workbench-chain-replay-4v.md"));
  const fullMatchWorkbenchChainReplay4W = readIfExists(join(shareDirectory, "fullmatch-workbench-chain-replay-4w.md"));
  ...
  const fullMatchWorkbenchChainReplay5E = readIfExists(join(shareDirectory, "fullmatch-workbench-chain-replay-5e.md"));
  const fullMatchWorkbenchChainReplay5EValidation = readIfExists(join(shareDirectory, "validation.fullmatch-workbench-chain-replay-5e.md"));
  const fullMatchWorkbenchChainReplay5F = readIfExists(join(shareDirectory, "fullmatch-workbench-chain-replay-5f.md"));
  ...
  const fullMatchWorkbenchChainReplay4SValidation = readIfExists(join(shareDirectory, "validation.fullmatch-workbench-chain-replay-4s.md"));
  ...
  const coachExportHtml = readIfExists(join(shareDirectory, "coach-report.export.html"));
  ...
  const coachExportAppendixStart = coachExportHtml.search(/<section\s+id="appendices"[^>]*>/u);
  const coachExportMainHtml = coachExportAppendixStart === -1
    ? coachExportHtml
    : coachExportHtml.slice(0, coachExportAppendixStart);
  ...
  const sprint4WForbiddenLeftovers = [
    "fullmatch-workbench-chain-replay-4v.md",
    "validation.fullmatch-workbench-chain-replay-4v.md",
    ...sprint4VForbiddenLeftovers,
  ];
  const sprint4XExpectedFiles = [
    "package.json",
    "tsconfig.json",
    "coach-report.latest.html",
    "coach-report.default.html",
  ...
  const sprint4XForbiddenLeftovers = [
    "fullmatch-workbench-chain-replay-4w.md",
    "validation.fullmatch-workbench-chain-replay-4w.md",
  ...
  const sprint4ZForbiddenLeftovers = [
    "fullmatch-workbench-chain-replay-4y.md",
    "validation.fullmatch-workbench-chain-replay-4y.md",
  ...
  const sprint5BExpectedFiles = sprint5AExpectedFiles.map((file) =>
    file === "fullmatch-workbench-chain-replay-5a.md"
      ? "fullmatch-workbench-chain-replay-5b.md"
      : file === "validation.fullmatch-workbench-chain-replay-5a.md"
        ? "validation.fullmatch-workbench-chain-replay-5b.md"
        : file
  );
  const sprint5BForbiddenLeftovers = [
    "fullmatch-workbench-chain-replay-5a.md",
    "validation.fullmatch-workbench-chain-replay-5a.md",
  ...
  const sprint5EExpectedFiles = sprint5DExpectedFiles.map((file) =>
    file === "fullmatch-workbench-chain-replay-5d.md"
      ? "fullmatch-workbench-chain-replay-5e.md"
      : file === "validation.fullmatch-workbench-chain-replay-5d.md"
        ? "validation.fullmatch-workbench-chain-replay-5e.md"
        : file
  );
  const sprint5EForbiddenLeftovers = [
    "fullmatch-workbench-chain-replay-5d.md",
    "validation.fullmatch-workbench-chain-replay-5d.md",
    ...sprint5DForbiddenLeftovers,
  ];
  const sprint5FExpectedFiles = sprint5EExpectedFiles.map((file) =>
    file === "fullmatch-workbench-chain-replay-5e.md"
      ? "fullmatch-workbench-chain-replay-5f.md"
      : file === "validation.fullmatch-workbench-chain-replay-5e.md"
        ? "validation.fullmatch-workbench-chain-replay-5f.md"
        : file
  );
  const sprint5FForbiddenLeftovers = [
    "fullmatch-workbench-chain-replay-5e.md",
    "validation.fullmatch-workbench-chain-replay-5e.md",
    ...sprint5EForbiddenLeftovers,
  ];
  const sprint5GExpectedFiles = sprint5FExpectedFiles.map((file) =>
    file === "fullmatch-workbench-chain-replay-5f.md"
      ? "fullmatch-workbench-chain-replay-5g.md"
      : file === "validation.fullmatch-workbench-chain-replay-5f.md"
        ? "validation.fullmatch-workbench-chain-replay-5g.md"
        : file
  );
  const sprint5GForbiddenLeftovers = [
    "fullmatch-workbench-chain-replay-5f.md",
    "validation.fullmatch-workbench-chain-replay-5f.md",
  ...
  const sprint6EExpectedFiles = sprint6DExpectedFiles.map((file) =>
    file === "fullmatch-official-scoring-connection-6d.md"
      ? "fullmatch-batch-economy-proof-6e.md"
      : file === "validation.fullmatch-official-scoring-connection-6d.md"
        ? "validation.fullmatch-batch-economy-proof-6e.md"
        : file
  );
  const sprint6EForbiddenLeftovers = [
    "fullmatch-official-scoring-connection-6d.md",
    "validation.fullmatch-official-scoring-connection-6d.md",
  ...
  const sprint4SForbiddenLeftovers = [
    "fullmatch-workbench-chain-replay-4r.md",
    "validation.fullmatch-workbench-chain-replay-4r.md",
    ...sprint4RForbiddenLeftovers,
  ];
  ...
  const sprint4XChecks: readonly SharePackCheck[] = [
    check("reports/share exists", existsSync(shareDirectory), shareDirectory),
    ...
    check("all expected files are copied", sprint4XExpectedFiles.every((file) => requiredCopied(file)), sprint4XExpectedFiles.filter((file) => !requiredCopied(file)).join(", ") || "all copied"),
    check("all expected files are listed in manifest", sprint4XExpectedFiles.every((file) => manifest.includes(file)), sprint4XExpectedFiles.filter((file) => !manifest.includes(file)).join(", ") || "all listed"),
    check("no stale files remain in reports/share", staleFiles.length === 0, staleFiles.join(", ") || "none"),
    check("excluded-by-default files are not in reports/share", excludedInShare.length === 0, excludedInShare.join(", ") || "none"),
    check("source reports were not deleted", missingExcludedSources.length === 0, missingExcludedSources.join(", ") || "0"),
    check("manifest exposes MINIMAL_REVIEW", manifest.includes("MINIMAL_REVIEW"), "mode visible"),
    check("manifest says upload every file in reports/share", manifest.includes("Upload every file in this reports/share directory."), "upload instruction visible"),
    check("current sprint is Sprint 4X", activeConfig.sprintName === "Sprint 4X - Multi-Match Phase Comparison", activeConfig.sprintName),
    check("share pack mode is MINIMAL_REVIEW", activeConfig.mode === "MINIMAL_REVIEW", activeConfig.mode),
    check("share pack under 20 files", filesOnDisk.length <= 20, String(filesOnDisk.length)),
    check("expected share file count is 20", filesOnDisk.length === 20, String(filesOnDisk.length)),
    check("missing expected files are none", sprint4XExpectedFiles.every((file) => requiredCopied(file)), sprint4XExpectedFiles.filter((file) => !requiredCopied(file)).join(", ") || "none"),
    check("previous sprint leftovers are 0", sprint4XForbiddenLeftovers.every((file) => !requiredCopied(file)), sprint4XForbiddenLeftovers.filter((file) => requiredCopied(file)).join(", ") || "0"),
    check("README is Sprint 4X oriented", readme.includes("# Sprint 4X Share Pack") && readme.includes("fullmatch-workbench-chain-replay-4x.md") && readme.includes("coach-report.export.html"), "README current"),
    check("4X report included", fullMatchWorkbenchChainReplay4X.includes("# FullMatch Workbench Chain Replay 4X") && fullMatchWorkbenchChainReplay4X.includes("Multi-Match Phase Comparison"), "4X doc included"),
    check("4X validation is PASS or PARTIAL", (fullMatchWorkbenchChainReplay4XValidation.includes("Status: PASS") || fullMatchWorkbenchChainReplay4XValidation.includes("Status: PARTIAL")) && fullMatchWorkbenchChainReplay4XValidation.includes("Multi-Match Phase Comparison status is available or partial"), "4X validation current"),
    ...
    check("phase stability section is present", coachExportHtml.includes("Stabilit&eacute; des signaux de phase") && coachExportHtml.includes("phase-stability-grid"), "phase stability visible"),
    check(
      "phase stability labels are present",
      (coachExportHtml.includes("Signal r&eacute;p&eacute;t&eacute") || coachExportHtml.includes("Signal répété")) &&
        coachExportHtml.includes("Visible dans ce run") &&
        (coachExportHtml.includes("Signal instable") ||
          coachExportHtml.includes("Donn&eacute;e insuffisante") ||
          coachExportHtml.includes("Donnée insuffisante")),
      "stability labels visible",
    ),
    check("local comparison guard remains visible", coachExportHtml.includes("Cette comparaison reste locale aux runs disponibles.") && fullMatchWorkbenchChainReplay4XValidation.includes("local comparison guard is visible"), "local guard visible"),
    check("premium export uses single source of truth", coachExportHtml.includes("Ce rapport export&eacute; reprend la lecture du rapport produit. Il ne cr&eacute;e pas une seconde source de v&eacute;rit&eacute;.") && fullMatchWorkbenchChainReplay4XValidation.includes("single source of truth"), "single source visible"),
    ...
    check("bundle includes multi-match phase comparison source files", bundleReports.includes("src/reports/coachReportMultiMatchPhaseComparison.ts") && bundleReports.includes("src/reports/buildCoachReportMultiMatchPhaseComparison.ts") && bundleReports.includes("src/reports/buildCoachReportMultiMatchPhaseComparisonSamples.ts"), "4X source bundled"),
    check("bundle includes multi-match phase comparison tests", bundleReports.includes("coachReportMultiMatchPhaseComparison.test.ts") && bundleReports.includes("phaseSignalStabilityRules.test.ts") && bundleReports.includes("coachReportMultiMatchPhaseComparisonRenderer.test.ts") && bundleReports.includes("coachReportMultiMatchPhaseComparisonSourceGuard.test.ts") && bundleReports.includes("coachReportMultiMatchPhaseComparisonCopy.test.ts") && bundleReports.includes("coachReportMultiMatchPhaseComparisonPrintCss.test.ts") && bundleReports.includes("coachReportMultiMatchPhaseComparisonGuard.test.ts"), "4X tests bundled"),
    check("simulation bundle includes scoring guard 4X", bundleSimulation.includes("scoringGuard.4x.test.ts") && bundleSimulation.includes("validateScoringGuard4X"), "4X scoring guard bundled"),
    check("comparison cannot mutate score", coachExportHtml.includes("score mutation count 0") || fullMatchWorkbenchChainReplay4XValidation.includes("comparison layer cannot mutate official score"), "score mutation forbidden"),
    check("comparison cannot mutate possession", coachExportHtml.includes("possession mutation count 0") || fullMatchWorkbenchChainReplay4XValidation.includes("comparison layer cannot mutate official possession"), "possession mutation forbidden"),
    check("comparison cannot create production scoring events", coachExportHtml.includes("production scoring event creation count 0") || fullMatchWorkbenchChainReplay4XValidation.includes("comparison layer cannot create production scoring events"), "production scoring creation forbidden"),
    check("comparison cannot claim global economy", fullMatchWorkbenchChainReplay4XValidation.includes("comparison layer cannot claim global economy"), "global economy forbidden"),
    check("no scoring constants changed", scoringEvents.includes("SHOT_GOAL") && scoringEvents.includes("TRY_TOUCHDOWN") && scoringEvents.includes("PENALTY_SHOT") && fullMatchWorkbenchChainReplay4XValidation.includes("scoring constants unchanged"), "scoring constants visible"),
    check("no MatchBonusEvent mutation", scoringEvents.includes("MatchBonusEvent") && scoringEvents.includes("not part of this live ScoringEvent stream") && fullMatchWorkbenchChainReplay4XValidation.includes("MatchBonusEvent unchanged"), "MatchBonusEvent separated"),
    check("batch/live separation preserved", scoringEvents.includes("batch/live separation status: PASS") && fullMatchWorkbenchChainReplay4XValidation.includes("FULL_MATCH_BATCH_ECONOMY remains the only global economy proof"), "batch/live PASS"),
  ...
  const sprint4ZChecks: readonly SharePackCheck[] = [
    check("reports/share exists", existsSync(shareDirectory), shareDirectory),
    check("manifest exists", manifest.length > 0, manifestPath),
    check("README exists", readme.length > 0, readmePath),
    check("detailed manifest exists", detailedManifest.length > 0, detailedManifestPath),
    check("validation.share-pack.md copied", sourceExists("validation.share-pack.md") && requiredCopied("validation.share-pack.md"), "validation.share-pack.md"),
    check("all expected files are copied", sprint4ZExpectedFiles.every((file) => requiredCopied(file)), sprint4ZExpectedFiles.filter((file) => !requiredCopied(file)).join(", ") || "all copied"),
    check("all expected files are listed in manifest", sprint4ZExpectedFiles.every((file) => manifest.includes(file)), sprint4ZExpectedFiles.filter((file) => !manifest.includes(file)).join(", ") || "all listed"),
    check("no stale files remain in reports/share", staleFiles.length === 0, staleFiles.join(", ") || "none"),
    check("excluded-by-default files are not in reports/share", excludedInShare.length === 0, excludedInShare.join(", ") || "none"),
    check("source reports were not deleted", missingExcludedSources.length === 0, missingExcludedSources.join(", ") || "0"),
    check("manifest exposes MINIMAL_REVIEW", manifest.includes("MINIMAL_REVIEW"), "mode visible"),
    check("manifest says upload every file in reports/share", manifest.includes("Upload every file in this reports/share directory."), "upload instruction visible"),
    check("current sprint is Sprint 4Z", activeConfig.sprintName === "Sprint 4Z - Real Match History Store & Report Integration", activeConfig.sprintName),
    check("share pack mode is MINIMAL_REVIEW", activeConfig.mode === "MINIMAL_REVIEW", activeConfig.mode),
    check("share pack under 20 files", filesOnDisk.length <= 20, String(filesOnDisk.length)),
    check("expected share file count is 20", filesOnDisk.length === 20, String(filesOnDisk.length)),
    check("missing expected files are none", sprint4ZExpectedFiles.every((file) => requiredCopied(file)), sprint4ZExpectedFiles.filter((file) => !requiredCopied(file)).join(", ") || "none"),
    check("previous sprint leftovers are 0", sprint4ZForbiddenLeftovers.every((file) => !requiredCopied(file)), sprint4ZForbiddenLeftovers.filter((file) => requiredCopied(file)).join(", ") || "0"),
    check("README is Sprint 4Z oriented", readme.includes("# Sprint 4Z Share Pack") && readme.includes("fullmatch-workbench-chain-replay-4z.md") && readme.includes("coach-report.export.html"), "README current"),
    check("4Z report included", fullMatchWorkbenchChainReplay4Z.includes("# FullMatch Workbench Chain Replay 4Z") && fullMatchWorkbenchChainReplay4Z.includes("Real Match History Store"), "4Z doc included"),
    check("4Z validation is PASS or PARTIAL", (fullMatchWorkbenchChainReplay4ZValidation.includes("Status: PASS") || fullMatchWorkbenchChainReplay4ZValidation.includes("Status: PARTIAL")) && fullMatchWorkbenchChainReplay4ZValidation.includes("Real Match History Store status is available or partial"), "4Z validation current"),
    check("product report HTML copied", coachProductHtml.includes("Rapport coach") && coachProductHtml.includes("Joueurs"), "product HTML visible"),
    check("export report HTML copied", coachExportHtml.includes("Rapport coach") && coachExportHtml.includes("data-export-snapshot=\"coach_product_report\""), "export HTML visible"),
    ...
    check("real match history section is present", coachExportHtml.includes("Historique produit des matchs") && coachExportHtml.includes("match-history-section") && coachExportHtml.includes("match-history-grid"), "real match history visible"),
    check("real match history boundary guard is present", coachExportHtml.includes("reste local et en lecture seule") && coachExportHtml.includes("aucune d&eacute;cision"), "history boundary visible"),
    check("real match history appendix is present", coachExportHtml.includes("D&eacute;tails du stockage d&rsquo;historique des matchs") || coachExportHtml.includes("Détails du stockage d"), "history appendix visible"),
    check("main export hides internal status names", !containsAny(coachExportMainHtml, ["officially_confirmed", "trace_supported", "sandbox_only"]), "internal statuses hidden"),
    check("main export avoids recommendation wording", !containsAny(coachExportMainHtml, ["meilleur choix", "composition recommand", "selection automatique", "preuve globale", "certitude"]), "recommendation wording count 0"),
    check("main export avoids selection wording", !containsAny(coachExportMainHtml, ["a selectionner", "player selected"]), "selection wording count 0"),
    check("visible French copy is clean", !containsAny(coachExportHtml, coachHtmlMojibakeMarkers), "mojibake count 0"),
    check(
      "bundle includes real match history source files",
    ...
    check(
      "bundle includes real match history tests",
    ...
    check("simulation bundle includes scoring guard 4Z", bundleSimulation.includes("scoringGuard.4z.test.ts"), "4Z scoring guard bundled"),
    check("history store cannot mutate score", coachExportHtml.includes("score mutation count 0") || fullMatchWorkbenchChainReplay4ZValidation.includes("history store cannot mutate official score"), "score mutation forbidden"),
    check("history store cannot mutate possession", coachExportHtml.includes("possession mutation count 0") || fullMatchWorkbenchChainReplay4ZValidation.includes("history store cannot mutate official possession"), "possession mutation forbidden"),
    check("history store cannot create production scoring events", coachExportHtml.includes("production scoring event creation count 0") || fullMatchWorkbenchChainReplay4ZValidation.includes("history store cannot create production scoring events"), "production scoring creation forbidden"),
    check("history store cannot claim global economy", fullMatchWorkbenchChainReplay4ZValidation.includes("history store cannot claim global economy"), "global economy forbidden"),
    check("no scoring constants changed", scoringEvents.includes("SHOT_GOAL") && scoringEvents.includes("TRY_TOUCHDOWN") && scoringEvents.includes("PENALTY_SHOT") && fullMatchWorkbenchChainReplay4ZValidation.includes("scoring constants unchanged"), "scoring constants visible"),
    check("no MatchBonusEvent mutation", scoringEvents.includes("MatchBonusEvent") && scoringEvents.includes("not part of this live ScoringEvent stream") && fullMatchWorkbenchChainReplay4ZValidation.includes("MatchBonusEvent unchanged"), "MatchBonusEvent separated"),
    check("batch/live separation preserved", scoringEvents.includes("batch/live separation status: PASS") && fullMatchWorkbenchChainReplay4ZValidation.includes("FULL_MATCH_BATCH_ECONOMY remains the only global economy proof"), "batch/live PASS"),
  ...
  const sprint5BChecks: readonly SharePackCheck[] = [
    check("reports/share exists", existsSync(shareDirectory), shareDirectory),
    check("manifest exists", manifest.length > 0, manifestPath),
    check("README exists", readme.length > 0, readmePath),
    check("detailed manifest exists", detailedManifest.length > 0, detailedManifestPath),
    check("validation.share-pack.md copied", sourceExists("validation.share-pack.md") && requiredCopied("validation.share-pack.md"), "validation.share-pack.md"),
    check("all expected files are copied", sprint5BExpectedFiles.every((file) => requiredCopied(file)), sprint5BExpectedFiles.filter((file) => !requiredCopied(file)).join(", ") || "all copied"),
    check("all expected files are listed in manifest", sprint5BExpectedFiles.every((file) => manifest.includes(file)), sprint5BExpectedFiles.filter((file) => !manifest.includes(file)).join(", ") || "all listed"),
    check("no stale files remain in reports/share", staleFiles.length === 0, staleFiles.join(", ") || "none"),
    check("excluded-by-default files are not in reports/share", excludedInShare.length === 0, excludedInShare.join(", ") || "none"),
    check("source reports were not deleted", missingExcludedSources.length === 0, missingExcludedSources.join(", ") || "0"),
    check("manifest exposes MINIMAL_REVIEW", manifest.includes("MINIMAL_REVIEW"), "mode visible"),
    check("manifest says upload every file in reports/share", manifest.includes("Upload every file in this reports/share directory."), "upload instruction visible"),
    check("current sprint is Sprint 5B", activeConfig.sprintName === "Sprint 5B - History Store Consistency & Database Adapter Contract", activeConfig.sprintName),
    check("share pack mode is MINIMAL_REVIEW", activeConfig.mode === "MINIMAL_REVIEW", activeConfig.mode),
    check("share pack under 20 files", filesOnDisk.length <= 20, String(filesOnDisk.length)),
    check("expected share file count is 20", filesOnDisk.length === 20, String(filesOnDisk.length)),
    check("missing expected files are none", sprint5BExpectedFiles.every((file) => requiredCopied(file)), sprint5BExpectedFiles.filter((file) => !requiredCopied(file)).join(", ") || "none"),
    check("previous sprint leftovers are 0", sprint5BForbiddenLeftovers.every((file) => !requiredCopied(file)), sprint5BForbiddenLeftovers.filter((file) => requiredCopied(file)).join(", ") || "0"),
    check("README is Sprint 5B oriented", readme.includes("# Sprint 5B Share Pack") && readme.includes("fullmatch-workbench-chain-replay-5b.md") && readme.includes("coach-report.export.html"), "README current"),
    check("5B report included", fullMatchWorkbenchChainReplay5B.includes("# FullMatch Workbench Chain Replay 5B") && fullMatchWorkbenchChainReplay5B.includes("History Store Consistency"), "5B doc included"),
    check("5B validation is PASS", fullMatchWorkbenchChainReplay5BValidation.includes("Status: PASS") && fullMatchWorkbenchChainReplay5BValidation.includes("History Store Consistency status is available or partial"), "5B validation current"),
    check("product report HTML copied", coachProductHtml.includes("Rapport coach") && coachProductHtml.includes("Joueurs"), "product HTML visible"),
    check("export report HTML copied", coachExportHtml.includes("Rapport coach") && coachExportHtml.includes("data-export-snapshot=\"coach_product_report\""), "export HTML visible"),
    check("persistent history subsection is present", coachExportHtml.includes("Persistance de l") && coachExportHtml.includes("persistent-history-section") && coachExportHtml.includes("persistent-history-grid"), "persistent history visible"),
    check("history store consistency subsection is present", coachExportHtml.includes("Coh&eacute;rence du stockage") && coachExportHtml.includes("history-consistency-section") && coachExportHtml.includes("history-consistency-grid"), "history consistency visible"),
    check("history store consistency appendix is present", coachExportHtml.includes("D&eacute;tails de coh&eacute;rence du stockage historique") && coachExportHtml.includes("save operation"), "history consistency appendix visible"),
    check("database adapter contract is visible", coachExportHtml.includes("Database adapter contract visible") && coachExportHtml.includes("implemented=false") && coachExportHtml.includes("migrationRequired=true"), "database contract visible"),
    check("main export hides internal status names", !containsAny(coachExportMainHtml, ["officially_confirmed", "trace_supported", "sandbox_only"]), "internal statuses hidden"),
    check("main export avoids recommendation wording", !containsAny(coachExportMainHtml, ["meilleur choix", "composition recommand", "selection automatique", "preuve globale", "certitude"]), "recommendation wording count 0"),
    check("main export avoids selection wording", !containsAny(coachExportMainHtml, ["a selectionner", "player selected"]), "selection wording count 0"),
    check("visible French copy is clean", !containsAny(coachExportHtml, coachHtmlMojibakeMarkers), "mojibake count 0"),
    check(
      "bundle includes history store consistency source files",
      bundleReports.includes("src/reports/history/coachMatchHistoryStore.ts") &&
        bundleReports.includes("CoachMatchHistorySaveResult") &&
        bundleReports.includes("src/reports/history/databaseCoachMatchHistoryAdapterContract.ts") &&
        bundleReports.includes("src/reports/coachReportHistoryStoreConsistency.ts") &&
        bundleReports.includes("src/reports/buildCoachReportHistoryStoreConsistency.ts"),
      "5B source bundled",
    ),
    check(
      "bundle includes history store consistency tests",
      bundleReports.includes("coachMatchHistorySaveResult.test.ts") &&
        bundleReports.includes("fileBackedCoachMatchHistoryStoreConsistency.test.ts") &&
        bundleReports.includes("inMemoryCoachMatchHistoryStoreConsistency.test.ts") &&
        bundleReports.includes("databaseCoachMatchHistoryAdapterContract.test.ts") &&
        bundleReports.includes("coachReportHistoryStoreConsistency.test.ts") &&
        bundleReports.includes("coachReportHistoryStoreConsistencyRenderer.test.ts") &&
        bundleReports.includes("coachReportHistoryStoreConsistencySourceGuard.test.ts") &&
        bundleReports.includes("coachReportHistoryStoreConsistencyCopy.test.ts") &&
        bundleReports.includes("coachReportHistoryStoreConsistencyPrintCss.test.ts") &&
        bundleReports.includes("coachReportHistoryStoreConsistencyGuard.test.ts"),
      "5B tests bundled",
    ),
    check("simulation bundle includes scoring guard 5B", bundleSimulation.includes("scoringGuard.5b.test.ts"), "5B scoring guard bundled"),
    check("save operation semantics are visible", fullMatchWorkbenchChainReplay5BValidation.includes("save operation is explicit") && fullMatchWorkbenchChainReplay5BValidation.includes("deduped record count is visible"), "save semantics visible"),
    check("database adapter implemented false is validated", fullMatchWorkbenchChainReplay5BValidation.includes("database adapter implemented is false"), "database contract guard visible"),
    check("history consistency cannot mutate score", fullMatchWorkbenchChainReplay5BValidation.includes("history consistency cannot mutate score"), "score mutation forbidden"),
    check("history consistency cannot create production scoring events", fullMatchWorkbenchChainReplay5BValidation.includes("history consistency cannot create production scoring events"), "production scoring creation forbidden"),
    check("history consistency cannot claim global economy", fullMatchWorkbenchChainReplay5BValidation.includes("history consistency cannot claim global economy"), "global economy forbidden"),
    check("no scoring constants changed", scoringEvents.includes("SHOT_GOAL") && scoringEvents.includes("TRY_TOUCHDOWN") && scoringEvents.includes("PENALTY_SHOT") && fullMatchWorkbenchChainReplay5BValidation.includes("scoring constants unchanged"), "scoring constants visible"),
    check("no MatchBonusEvent mutation", scoringEvents.includes("MatchBonusEvent") && scoringEvents.includes("not part of this live ScoringEvent stream") && fullMatchWorkbenchChainReplay5BValidation.includes("MatchBonusEvent unchanged"), "MatchBonusEvent separated"),
    check("batch/live separation preserved", scoringEvents.includes("batch/live separation status: PASS") && fullMatchWorkbenchChainReplay5BValidation.includes("FULL_MATCH_BATCH_ECONOMY remains the only global economy proof"), "batch/live PASS"),
  ...
  const sprint6EChecks: readonly SharePackCheck[] = [
    ...
    check("all expected files are copied", sprint6EExpectedFiles.every((file) => requiredCopied(file)), sprint6EExpectedFiles.filter((file) => !requiredCopied(file)).join(", ") || "all copied"),
    check("all expected files are listed in manifest", sprint6EExpectedFiles.every((file) => manifest.includes(file)), sprint6EExpectedFiles.filter((file) => !manifest.includes(file)).join(", ") || "all listed"),
    ...
    check("current sprint is Sprint 6E", activeConfig.sprintName === "Sprint 6E - Full-Match Batch Economy Proof", activeConfig.sprintName),
    check("missing expected files are none", sprint6EExpectedFiles.every((file) => requiredCopied(file)), sprint6EExpectedFiles.filter((file) => !requiredCopied(file)).join(", ") || "none"),
    check("previous sprint leftovers are 0", sprint6EForbiddenLeftovers.every((file) => !requiredCopied(file)), sprint6EForbiddenLeftovers.filter((file) => requiredCopied(file)).join(", ") || "0"),
    check("README is Sprint 6E oriented", readme.includes("# Sprint 6E Share Pack") && readme.includes("fullmatch-batch-economy-proof-6e.md") && readme.includes("coach-report.export.html"), "README current"),
    check("6E report included", fullMatchBatchEconomyProof6E.includes("# Full-Match Batch Economy Proof 6E") && fullMatchBatchEconomyProof6E.includes("Scoreline Distribution") && fullMatchBatchEconomyProof6E.includes("Scoring Points Share By Family"), "6E doc included"),
    check("6E validation is PASS", fullMatchBatchEconomyProof6EValidation.includes("Status: PASS") && fullMatchBatchEconomyProof6EValidation.includes("matchCount >= 50") && fullMatchBatchEconomyProof6EValidation.includes("explicit exhaustive test command is available"), "6E validation current"),
    check("batch match count visible", fullMatchBatchEconomyProof6E.includes("matchCount: 50") && fullMatchBatchEconomyProof6EValidation.includes("matchCount: 50"), "50 matches visible"),
    check("seed variation visible", fullMatchBatchEconomyProof6E.includes("uniqueSeeds: 50") && fullMatchBatchEconomyProof6EValidation.includes("seed variation exists"), "unique seeds visible"),
    check("scoreline diversity visible", fullMatchBatchEconomyProof6E.includes("uniqueScorelines:") && fullMatchBatchEconomyProof6E.includes("| Scoreline | Matches |"), "scorelines visible"),
    check("scoring family distribution visible", fullMatchBatchEconomyProof6E.includes("events SHOT_GOAL:") && fullMatchBatchEconomyProof6E.includes("points SHOT_GOAL:") && fullMatchBatchEconomyProof6E.includes("share percent SHOT_GOAL:"), "family distribution visible"),
    check("blowout and severe blowout rates visible", fullMatchBatchEconomyProof6E.includes("blowoutRate:") && fullMatchBatchEconomyProof6E.includes("severeBlowoutRate:"), "blowout rates visible"),
    check("official path connected all runs", fullMatchBatchEconomyProof6E.includes("officialPathConnectedAllRuns: true") && fullMatchBatchEconomyProof6EValidation.includes("official path connected in all runs"), "official path all runs"),
    check("calibrations applied all runs", fullMatchBatchEconomyProof6E.includes("calibrationAppliedAllRuns: true") && fullMatchBatchEconomyProof6EValidation.includes("calibrations applied in all runs"), "calibrations all runs"),
    check("score from score_change all runs", fullMatchBatchEconomyProof6E.includes("scoreFromScoreChangeAllRuns: true") && fullMatchBatchEconomyProof6EValidation.includes("score from score_change in all runs"), "score_change source all runs"),
    check("no score cap", fullMatchBatchEconomyProof6E.includes("scoreCapAppliedCount: 0") && fullMatchBatchEconomyProof6EValidation.includes("no score cap"), "cap 0"),
    check("no post-hoc rewrite", fullMatchBatchEconomyProof6E.includes("postHocRewriteCount: 0") && fullMatchBatchEconomyProof6EValidation.includes("no post-hoc rewrite"), "rewrite 0"),
    check("no event deletion", fullMatchBatchEconomyProof6E.includes("scoringEventDeletionCount: 0") && fullMatchBatchEconomyProof6EValidation.includes("no event deletion"), "deletion 0"),
    check("no forced opponent score", fullMatchBatchEconomyProof6E.includes("forcedOpponentScoreCount: 0") && fullMatchBatchEconomyProof6EValidation.includes("no forced opponent score"), "forced score 0"),
    check("scoring constants unchanged", scoringEvents.includes("SHOT_GOAL") && scoringEvents.includes("TRY_TOUCHDOWN") && scoringEvents.includes("DROP_GOAL") && scoringEvents.includes("PENALTY_SHOT") && fullMatchBatchEconomyProof6E.includes("scoringConstantsChangedCount: 0"), "scoring constants visible"),
    check("MatchBonusEvent unchanged", scoringEvents.includes("MatchBonusEvent") && scoringEvents.includes("not part of this live ScoringEvent stream") && fullMatchBatchEconomyProof6E.includes("MatchBonusEventChangedCount: 0"), "MatchBonusEvent separated"),
    check("batch/live separation preserved", scoringEvents.includes("batch/live separation status: PASS") && fullMatchBatchEconomyProof6E.includes("batchLiveContaminationCount: 0"), "batch/live PASS"),
    check("persistence and SQLite not used for scoring", fullMatchBatchEconomyProof6E.includes("persistenceUsedForScoringCount: 0") && fullMatchBatchEconomyProof6E.includes("sqliteUsedForScoringCount: 0"), "persistence/SQLite 0"),
    check("no UNKNOWN scoring family", fullMatchBatchEconomyProof6E.includes("unknownScoringFamilyCount: 0") && fullMatchBatchEconomyProof6EValidation.includes("no UNKNOWN scoring family"), "UNKNOWN 0"),
    check("no PENALTY_SHOT leakage", fullMatchBatchEconomyProof6E.includes("penaltyShotActiveLeakageCount: 0") && fullMatchBatchEconomyProof6EValidation.includes("no PENALTY_SHOT leakage"), "PENALTY 0"),
    check("PARTIAL/PASS economy status is justified", (fullMatchBatchEconomyProof6E.includes("status: PARTIAL") || fullMatchBatchEconomyProof6E.includes("status: PASS")) && fullMatchBatchEconomyProof6EValidation.includes("PASS/PARTIAL/FAIL status justified"), "status justified"),
    check("economy warnings visible", fullMatchBatchEconomyProof6E.includes("FULL_MATCH_BATCH_ECONOMY_NEEDS_TARGETED_FIX") || fullMatchBatchEconomyProof6E.includes("FULL_MATCH_BATCH_ECONOMY_CONFIRMED"), "economy warning visible"),
    check("coach product contains batch proof section", coachProductHtml.includes("Preuve batch full-match") && (coachProductHtml.includes("Économie observée") || coachProductHtml.includes("&Eacute;conomie observ&eacute;e")), "product 6E visible"),
    check("coach export contains batch proof section", coachExportHtml.includes("Preuve batch full-match") && (coachExportHtml.includes("Économie observée") || coachExportHtml.includes("&Eacute;conomie observ&eacute;e")), "export 6E visible"),
    check("coach export avoids forbidden guarantee wording", !coachExportHtml.includes("equilibre garanti") && !coachExportHtml.includes("economie parfaite") && !coachExportHtml.includes("preuve definitive") && !coachExportHtml.includes("score corrig") && !coachExportHtml.includes("score ajust"), "forbidden wording absent"),
    check("bundle includes 6E source files", bundleSimulation.includes("src/reports/fullMatchBatchEconomyWarnings.ts") && bundleSimulation.includes("src/reports/fullMatchBatchEconomyProof.ts") && bundleSimulation.includes("src/reports/fullMatchBatchEconomyProof.test.ts") && bundleSimulation.includes("buildFullMatchBatchEconomyProofModel"), "6E source bundled"),
    check("bundle includes official scoring path regression test", bundleSimulation.includes("fullMatchOfficialScoringPath.test.ts") && bundleSimulation.includes("rejects conversion score_change when its segment try was not accepted"), "conversion/try test bundled"),
    check("explicit exhaustive test command available", readIfExists(join(shareDirectory, "package.json")).includes("\"test:all\"") && fullMatchBatchEconomyProof6EValidation.includes("npm run build && npm run typecheck && npm run test:contracts && npm run test:all && npm run reports:coach && npm run reports:share"), "test:all visible"),
    check("recommendation visible", fullMatchBatchEconomyProof6E.includes("recommendation:") && fullMatchBatchEconomyProof6E.includes("nextSprintRecommendation:"), "6E recommendation visible"),
  ];
  ...
  const sprint5GChecks: readonly SharePackCheck[] = [
    ...
    check("detailed manifest exists", detailedManifest.length > 0, detailedManifestPath),
    check("validation.share-pack.md copied", sourceExists("validation.share-pack.md") && requiredCopied("validation.share-pack.md"), "validation.share-pack.md"),
    check("all expected files are copied", sprint5GExpectedFiles.every((file) => requiredCopied(file)), sprint5GExpectedFiles.filter((file) => !requiredCopied(file)).join(", ") || "all copied"),
    check("all expected files are listed in manifest", sprint5GExpectedFiles.every((file) => manifest.includes(file)), sprint5GExpectedFiles.filter((file) => !manifest.includes(file)).join(", ") || "all listed"),
    check("no stale files remain in reports/share", staleFiles.length === 0, staleFiles.join(", ") || "none"),
    check("excluded-by-default files are not in reports/share", excludedInShare.length === 0, excludedInShare.join(", ") || "none"),
    check("source reports were not deleted", missingExcludedSources.length === 0, missingExcludedSources.join(", ") || "0"),
    check("manifest exposes MINIMAL_REVIEW", manifest.includes("MINIMAL_REVIEW"), "mode visible"),
    check("manifest says upload every file in reports/share", manifest.includes("Upload every file in this reports/share directory."), "upload instruction visible"),
    check("current sprint is Sprint 5G", activeConfig.sprintName === "Sprint 5G - Controlled Local Read-Only DB Mode", activeConfig.sprintName),
    check("share pack mode is MINIMAL_REVIEW", activeConfig.mode === "MINIMAL_REVIEW", activeConfig.mode),
    check("share pack under 20 files", filesOnDisk.length <= 20, String(filesOnDisk.length)),
    check("expected share file count is 20", filesOnDisk.length === 20, String(filesOnDisk.length)),
    check("missing expected files are none", sprint5GExpectedFiles.every((file) => requiredCopied(file)), sprint5GExpectedFiles.filter((file) => !requiredCopied(file)).join(", ") || "none"),
    check("previous sprint leftovers are 0", sprint5GForbiddenLeftovers.every((file) => !requiredCopied(file)), sprint5GForbiddenLeftovers.filter((file) => requiredCopied(file)).join(", ") || "0"),
    check("README is Sprint 5G oriented", readme.includes("# Sprint 5G Share Pack") && readme.includes("fullmatch-workbench-chain-replay-5g.md") && readme.includes("coach-report.export.html"), "README current"),
    check("5G report included", fullMatchWorkbenchChainReplay5G.includes("# FullMatch Workbench Chain Replay 5G") && fullMatchWorkbenchChainReplay5G.includes("Controlled Local Read-Only DB Mode") && fullMatchWorkbenchChainReplay5G.includes("Product Boundary"), "5G doc included"),
    check("5G validation is PASS", fullMatchWorkbenchChainReplay5GValidation.includes("Status: PASS") && fullMatchWorkbenchChainReplay5GValidation.includes("controlled local read-only DB mode exists") && fullMatchWorkbenchChainReplay5GValidation.includes("schema version is coach_match_history_v1"), "5G validation current"),
    check("product report HTML copied", coachProductHtml.includes("Rapport coach") && coachProductHtml.includes("Joueurs"), "product HTML visible"),
    check("export report HTML copied", coachExportHtml.includes("Rapport coach") && coachExportHtml.includes("data-export-snapshot=\"coach_product_report\""), "export HTML visible"),
    check("controlled read-only section is present", coachExportHtml.includes("Lecture SQLite locale contr&ocirc;l&eacute;e") && coachExportHtml.includes("controlled-local-readonly-db-section"), "controlled read-only section visible"),
    check("sqlite local target visible", fullMatchWorkbenchChainReplay5G.includes("storage target selected: sqlite_local") && coachExportHtml.includes("sqlite_local"), "sqlite_local visible"),
    check("schema version visible", fullMatchWorkbenchChainReplay5G.includes("schema version: coach_match_history_v1") && coachExportHtml.includes("coach_match_history_v1"), "schema visible"),
    check("read-only mode true", fullMatchWorkbenchChainReplay5GValidation.includes("read-only mode is true") && coachExportHtml.includes("Read-only mode"), "read-only visible"),
    check("write mode disabled", fullMatchWorkbenchChainReplay5GValidation.includes("write mode allowed is false") && fullMatchWorkbenchChainReplay5GValidation.includes("write rejected passes"), "write rejection visible"),
    check("product database activation remains disabled", fullMatchWorkbenchChainReplay5G.includes("product activation allowed: false") && fullMatchWorkbenchChainReplay5G.includes("database used as product truth: false"), "product DB activation disabled"),
    check("file_backed remains active product source", fullMatchWorkbenchChainReplay5G.includes("active product history source: file_backed") && coachExportHtml.includes("file_backed"), "file_backed visible"),
    check("real database read and write counts are 0", fullMatchWorkbenchChainReplay5GValidation.includes("real DB write count is 0") && fullMatchWorkbenchChainReplay5GValidation.includes("default real DB read count is 0") && coachExportHtml.includes("Lectures DB r&eacute;elles mode d&eacute;faut"), "real DB counters 0"),
    check("controlled read attempts are visible", fullMatchWorkbenchChainReplay5GValidation.includes("controlled read attempts are visible") && fullMatchWorkbenchChainReplay5G.includes("controlled read attempt count:"), "controlled reads visible"),
    check("query by team and phase pass", fullMatchWorkbenchChainReplay5GValidation.includes("query by team passes") && fullMatchWorkbenchChainReplay5GValidation.includes("query by phase passes"), "read-only queries visible"),
    check("deterministic ordering and schema compatibility pass", fullMatchWorkbenchChainReplay5GValidation.includes("deterministic ordering passes") && fullMatchWorkbenchChainReplay5GValidation.includes("schema compatibility passes"), "schema/order visible"),
    check("bundle includes controlled read-only source files", bundleReports.includes("src/reports/history/sqliteLocalReadOnlyCoachMatchHistoryAdapter.ts") && bundleReports.includes("src/reports/coachReportControlledLocalReadOnlyDbMode.ts") && bundleReports.includes("src/reports/buildCoachReportControlledLocalReadOnlyDbMode.ts"), "5G source bundled"),
    check("bundle includes controlled read-only tests", bundleReports.includes("sqliteLocalReadOnlyCoachMatchHistoryAdapter.test.ts") && bundleReports.includes("coachReportControlledLocalReadOnlyDbModeRenderer.test.ts") && bundleReports.includes("coachReportControlledLocalReadOnlyDbModeGuard.test.ts"), "5G tests bundled"),
    check("simulation bundle includes scoring guard 5G", bundleSimulation.includes("scoringGuard.5g.test.ts"), "5G scoring guard bundled"),
    check("controlled read-only evidence category bundled", bundleContracts.includes("WORKBENCH_CHAIN_COACH_REPORT_CONTROLLED_LOCAL_READONLY_DB_MODE") && bundleSimulation.includes("WORKBENCH_CHAIN_COACH_REPORT_CONTROLLED_LOCAL_READONLY_DB_MODE"), "5G evidence category bundled"),
    check("main export avoids recommendation wording", !containsAny(coachExportMainHtml, ["meilleur choix", "composition recommand", "selection automatique", "preuve globale", "certitude"]), "recommendation wording count 0"),
    check("main export avoids selection wording", !containsAny(coachExportMainHtml, ["a selectionner", "player selected"]), "selection wording count 0"),
    check("visible French copy is clean", !containsAny(coachExportHtml, coachHtmlMojibakeMarkers), "mojibake count 0"),
    check("score mutation count is 0", fullMatchWorkbenchChainReplay5GValidation.includes("score mutation count is 0"), "score mutation 0"),
    check("timeline mutation count is 0", fullMatchWorkbenchChainReplay5GValidation.includes("timeline mutation count is 0"), "timeline mutation 0"),
    check("possession mutation count is 0", fullMatchWorkbenchChainReplay5GValidation.includes("possession mutation count is 0"), "possession mutation 0"),
    check("production scoring event creation count is 0", fullMatchWorkbenchChainReplay5GValidation.includes("production scoring event creation count is 0"), "scoring event creation 0"),
    check("lineup/starters/bench mutation counts are 0", fullMatchWorkbenchChainReplay5GValidation.includes("lineup mutation count is 0") && fullMatchWorkbenchChainReplay5GValidation.includes("starters mutation count is 0") && fullMatchWorkbenchChainReplay5GValidation.includes("bench mutation count is 0"), "lineup guard 0"),
    check("live selection and production route driver counts are 0", fullMatchWorkbenchChainReplay5GValidation.includes("live selection driver count is 0") && fullMatchWorkbenchChainReplay5GValidation.includes("production route resolution driver count is 0"), "selection/route drivers 0"),
    check("trend proof and invented statistic counts are 0", fullMatchWorkbenchChainReplay5GValidation.includes("trend proof claim count is 0") && fullMatchWorkbenchChainReplay5GValidation.includes("invented statistic count is 0"), "proof/invented counts 0"),
    check("sandbox events promoted count is 0", fullMatchWorkbenchChainReplay5GValidation.includes("sandbox events promoted to official count is 0"), "sandbox promotion 0"),
    check("no scoring constants changed", scoringEvents.includes("SHOT_GOAL") && scoringEvents.includes("TRY_TOUCHDOWN") && scoringEvents.includes("PENALTY_SHOT") && fullMatchWorkbenchChainReplay5GValidation.includes("scoring constants unchanged"), "scoring constants visible"),
    check("no MatchBonusEvent mutation", scoringEvents.includes("MatchBonusEvent") && scoringEvents.includes("not part of this live ScoringEvent stream") && fullMatchWorkbenchChainReplay5GValidation.includes("MatchBonusEvent unchanged"), "MatchBonusEvent separated"),
    check("batch/live separation preserved", scoringEvents.includes("batch/live separation status: PASS") && fullMatchWorkbenchChainReplay5GValidation.includes("FULL_MATCH_BATCH_ECONOMY remains only global economy proof"), "batch/live PASS"),
    check("50-match economy remains global reference", fullMatchWorkbenchChainReplay5G.includes("FULL_MATCH_BATCH_ECONOMY remains the only global economy proof") && bundleSimulation.includes("VALIDATED_FULL_MATCH_ECONOMY_ANCHOR"), "50-match reference visible"),
    check("explicit exhaustive test command available", readIfExists(join(shareDirectory, "package.json")).includes("\"test:all\"") && fullMatchWorkbenchChainReplay5GValidation.includes("explicit exhaustive test command is available"), "test:all visible"),
    check("recommendations visible", fullMatchWorkbenchChainReplay5GValidation.includes("CONFIRM_CONTROLLED_LOCAL_READONLY_DB_MODE") && fullMatchWorkbenchChainReplay5GValidation.includes("CONFIRM_NO_DB_WRITES") && fullMatchWorkbenchChainReplay5GValidation.includes("PREPARE_PRODUCT_HISTORY_SOURCE_SWITCH_TRIAL_NON_PROD_ONLY"), "5G recommendations visible"),
  ];

  const sprint5FChecks: readonly SharePackCheck[] = [
    check("reports/share exists", existsSync(shareDirectory), shareDirectory),
    check("manifest exists", manifest.length > 0, manifestPath),
    check("README exists", readme.length > 0, readmePath),
    check("detailed manifest exists", detailedManifest.length > 0, detailedManifestPath),
    check("validation.share-pack.md copied", sourceExists("validation.share-pack.md") && requiredCopied("validation.share-pack.md"), "validation.share-pack.md"),
    check("all expected files are copied", sprint5FExpectedFiles.every((file) => requiredCopied(file)), sprint5FExpectedFiles.filter((file) => !requiredCopied(file)).join(", ") || "all copied"),
    check("all expected files are listed in manifest", sprint5FExpectedFiles.every((file) => manifest.includes(file)), sprint5FExpectedFiles.filter((file) => !manifest.includes(file)).join(", ") || "all listed"),
    check("no stale files remain in reports/share", staleFiles.length === 0, staleFiles.join(", ") || "none"),
    check("excluded-by-default files are not in reports/share", excludedInShare.length === 0, excludedInShare.join(", ") || "none"),
    check("source reports were not deleted", missingExcludedSources.length === 0, missingExcludedSources.join(", ") || "0"),
    check("manifest exposes MINIMAL_REVIEW", manifest.includes("MINIMAL_REVIEW"), "mode visible"),
    check("manifest says upload every file in reports/share", manifest.includes("Upload every file in this reports/share directory."), "upload instruction visible"),
    check("current sprint is Sprint 5F", activeConfig.sprintName === "Sprint 5F - Durable Storage Decision & Disabled Real Adapter Wiring", activeConfig.sprintName),
    check("share pack mode is MINIMAL_REVIEW", activeConfig.mode === "MINIMAL_REVIEW", activeConfig.mode),
    check("share pack under 20 files", filesOnDisk.length <= 20, String(filesOnDisk.length)),
    check("expected share file count is 20", filesOnDisk.length === 20, String(filesOnDisk.length)),
    check("missing expected files are none", sprint5FExpectedFiles.every((file) => requiredCopied(file)), sprint5FExpectedFiles.filter((file) => !requiredCopied(file)).join(", ") || "none"),
    check("previous sprint leftovers are 0", sprint5FForbiddenLeftovers.every((file) => !requiredCopied(file)), sprint5FForbiddenLeftovers.filter((file) => requiredCopied(file)).join(", ") || "0"),
    check("README is Sprint 5F oriented", readme.includes("# Sprint 5F Share Pack") && readme.includes("fullmatch-workbench-chain-replay-5f.md") && readme.includes("coach-report.export.html"), "README current"),
    check("5F report included", fullMatchWorkbenchChainReplay5F.includes("# FullMatch Workbench Chain Replay 5F") && fullMatchWorkbenchChainReplay5F.includes("Durable Storage Decision") && fullMatchWorkbenchChainReplay5F.includes("Schema Decision"), "5F doc included"),
    check("5F validation is PASS", fullMatchWorkbenchChainReplay5FValidation.includes("Status: PASS") && fullMatchWorkbenchChainReplay5FValidation.includes("storage target selected is sqlite_local") && fullMatchWorkbenchChainReplay5FValidation.includes("schema version is coach_match_history_v1"), "5F validation current"),
    check("product report HTML copied", coachProductHtml.includes("Rapport coach") && coachProductHtml.includes("Joueurs"), "product HTML visible"),
    check("export report HTML copied", coachExportHtml.includes("Rapport coach") && coachExportHtml.includes("data-export-snapshot=\"coach_product_report\""), "export HTML visible"),
    check("durable storage decision section is present", coachExportHtml.includes("D&eacute;cision stockage durable") && coachExportHtml.includes("durable-storage-decision-section"), "durable storage section visible"),
    check("durable storage appendix is present", coachExportHtml.includes("D&eacute;tails d&eacute;cision stockage durable") && coachExportHtml.includes("schema version: coach_match_history_v1"), "durable storage appendix visible"),
    check("legacy migration wording clarified", coachExportHtml.includes("previous migration SPI") && !coachExportHtml.includes("Database adapter implemented false, production ready false"), "legacy wording clarified"),
    check("sqlite local target visible", fullMatchWorkbenchChainReplay5F.includes("storage target selected: sqlite_local") && coachExportHtml.includes("sqlite_local"), "sqlite_local visible"),
    check("schema version visible", fullMatchWorkbenchChainReplay5F.includes("schema version: coach_match_history_v1") && coachExportHtml.includes("coach_match_history_v1"), "schema visible"),
    check("real adapter wiring prepared", fullMatchWorkbenchChainReplay5FValidation.includes("real adapter wiring prepared") && coachExportHtml.includes("Real adapter wiring prepared"), "real adapter wiring visible"),
    check("product database activation remains disabled", fullMatchWorkbenchChainReplay5F.includes("production activation allowed: false") && fullMatchWorkbenchChainReplay5F.includes("database used as product truth: false"), "product DB activation disabled"),
    check("real database read and write counts are 0", fullMatchWorkbenchChainReplay5FValidation.includes("real DB write count is 0") && fullMatchWorkbenchChainReplay5FValidation.includes("real DB read count is 0") && coachExportHtml.includes("Real DB read count"), "real DB counters 0"),
    check("save-result scenarios pass", fullMatchWorkbenchChainReplay5FValidation.includes("inserted scenario passes") && fullMatchWorkbenchChainReplay5FValidation.includes("replaced scenario passes") && fullMatchWorkbenchChainReplay5FValidation.includes("ignored duplicate scenario passes"), "save-result scenarios visible"),
    check("report queries remain dry-run", fullMatchWorkbenchChainReplay5FValidation.includes("query by team passes") && fullMatchWorkbenchChainReplay5FValidation.includes("query by phase passes"), "dry-run query guard visible"),
    check("bundle includes durable storage source files", bundleReports.includes("src/reports/history/coachMatchHistoryDurableSchema.ts") && bundleReports.includes("src/reports/history/sqliteLocalCoachMatchHistoryAdapter.ts") && bundleReports.includes("src/reports/coachReportDurableStorageDecision.ts") && bundleReports.includes("src/reports/buildCoachReportDurableStorageDecision.ts"), "5F source bundled"),
    check("bundle includes durable storage tests", bundleReports.includes("coachMatchHistoryDurableSchema.test.ts") && bundleReports.includes("sqliteLocalCoachMatchHistoryAdapter.test.ts") && bundleReports.includes("coachReportDurableStorageDecisionRenderer.test.ts") && bundleReports.includes("coachReportDurableStorageDecisionGuard.test.ts"), "5F tests bundled"),
    check("simulation bundle includes scoring guard 5F", bundleSimulation.includes("scoringGuard.5f.test.ts"), "5F scoring guard bundled"),
    check("durable storage evidence category bundled", bundleContracts.includes("WORKBENCH_CHAIN_COACH_REPORT_DURABLE_STORAGE_DECISION") && bundleSimulation.includes("WORKBENCH_CHAIN_COACH_REPORT_DURABLE_STORAGE_DECISION"), "5F evidence category bundled"),
    check("main export hides internal status names", !containsAny(coachExportMainHtml, ["officially_confirmed", "trace_supported", "sandbox_only"]), "internal statuses hidden"),
    check("main export avoids recommendation wording", !containsAny(coachExportMainHtml, ["meilleur choix", "composition recommand", "selection automatique", "preuve globale", "certitude"]), "recommendation wording count 0"),
    check("main export avoids selection wording", !containsAny(coachExportMainHtml, ["a selectionner", "player selected"]), "selection wording count 0"),
    check("visible French copy is clean", !containsAny(coachExportHtml, coachHtmlMojibakeMarkers), "mojibake count 0"),
    check("no scoring constants changed", scoringEvents.includes("SHOT_GOAL") && scoringEvents.includes("TRY_TOUCHDOWN") && scoringEvents.includes("PENALTY_SHOT") && fullMatchWorkbenchChainReplay5FValidation.includes("scoring constants unchanged"), "scoring constants visible"),
    check("no MatchBonusEvent mutation", scoringEvents.includes("MatchBonusEvent") && scoringEvents.includes("not part of this live ScoringEvent stream") && fullMatchWorkbenchChainReplay5FValidation.includes("MatchBonusEvent unchanged"), "MatchBonusEvent separated"),
    check("batch/live separation preserved", scoringEvents.includes("batch/live separation status: PASS") && fullMatchWorkbenchChainReplay5FValidation.includes("FULL_MATCH_BATCH_ECONOMY remains the only global economy proof"), "batch/live PASS"),
    check("50-match economy remains global reference", fullMatchWorkbenchChainReplay5F.includes("FULL_MATCH_BATCH_ECONOMY remains the only global economy proof") && bundleSimulation.includes("VALIDATED_FULL_MATCH_ECONOMY_ANCHOR"), "50-match reference visible"),
    check("explicit exhaustive test command available", readIfExists(join(shareDirectory, "package.json")).includes("\"test:all\"") && fullMatchWorkbenchChainReplay5FValidation.includes("explicit exhaustive test command is available"), "test:all visible"),
    check("recommendations visible", fullMatchWorkbenchChainReplay5FValidation.includes("CONFIRM_DURABLE_STORAGE_DECISION_SQLITE_LOCAL") && fullMatchWorkbenchChainReplay5FValidation.includes("CONFIRM_NO_PRODUCT_DATABASE_ACTIVATION") && fullMatchWorkbenchChainReplay5FValidation.includes("PREPARE_CONTROLLED_LOCAL_TEST_READ_ONLY_DB_MODE"), "5F recommendations visible"),
  ];
  const sprint5EChecks: readonly SharePackCheck[] = [
    check("reports/share exists", existsSync(shareDirectory), shareDirectory),
    check("manifest exists", manifest.length > 0, manifestPath),
    check("README exists", readme.length > 0, readmePath),
    check("detailed manifest exists", detailedManifest.length > 0, detailedManifestPath),
    check("validation.share-pack.md copied", sourceExists("validation.share-pack.md") && requiredCopied("validation.share-pack.md"), "validation.share-pack.md"),
    check("all expected files are copied", sprint5EExpectedFiles.every((file) => requiredCopied(file)), sprint5EExpectedFiles.filter((file) => !requiredCopied(file)).join(", ") || "all copied"),
    check("all expected files are listed in manifest", sprint5EExpectedFiles.every((file) => manifest.includes(file)), sprint5EExpectedFiles.filter((file) => !manifest.includes(file)).join(", ") || "all listed"),
    check("no stale files remain in reports/share", staleFiles.length === 0, staleFiles.join(", ") || "none"),
    check("excluded-by-default files are not in reports/share", excludedInShare.length === 0, excludedInShare.join(", ") || "none"),
    check("source reports were not deleted", missingExcludedSources.length === 0, missingExcludedSources.join(", ") || "0"),
    check("manifest exposes MINIMAL_REVIEW", manifest.includes("MINIMAL_REVIEW"), "mode visible"),
    check("manifest says upload every file in reports/share", manifest.includes("Upload every file in this reports/share directory."), "upload instruction visible"),
    check("current sprint is Sprint 5E", activeConfig.sprintName === "Sprint 5E - Database Adapter Implementation Spike Without Product Activation", activeConfig.sprintName),
    check("share pack mode is MINIMAL_REVIEW", activeConfig.mode === "MINIMAL_REVIEW", activeConfig.mode),
    check("share pack under 20 files", filesOnDisk.length <= 20, String(filesOnDisk.length)),
    check("expected share file count is 20", filesOnDisk.length === 20, String(filesOnDisk.length)),
    check("missing expected files are none", sprint5EExpectedFiles.every((file) => requiredCopied(file)), sprint5EExpectedFiles.filter((file) => !requiredCopied(file)).join(", ") || "none"),
    check("previous sprint leftovers are 0", sprint5EForbiddenLeftovers.every((file) => !requiredCopied(file)), sprint5EForbiddenLeftovers.filter((file) => requiredCopied(file)).join(", ") || "0"),
    check("README is Sprint 5E oriented", readme.includes("# Sprint 5E Share Pack") && readme.includes("fullmatch-workbench-chain-replay-5e.md") && readme.includes("coach-report.export.html"), "README current"),
    check("5E report included", fullMatchWorkbenchChainReplay5E.includes("# FullMatch Workbench Chain Replay 5E") && fullMatchWorkbenchChainReplay5E.includes("Experimental Database Adapter") && fullMatchWorkbenchChainReplay5E.includes("Spike Scenarios"), "5E doc included"),
    check("5E validation is PASS", fullMatchWorkbenchChainReplay5EValidation.includes("Status: PASS") && fullMatchWorkbenchChainReplay5EValidation.includes("Database Adapter Spike model is available") && fullMatchWorkbenchChainReplay5EValidation.includes("adapter kind is experimental_database"), "5E validation current"),
    check("product report HTML copied", coachProductHtml.includes("Rapport coach") && coachProductHtml.includes("Joueurs"), "product HTML visible"),
    check("export report HTML copied", coachExportHtml.includes("Rapport coach") && coachExportHtml.includes("data-export-snapshot=\"coach_product_report\""), "export HTML visible"),
    check("experimental database adapter section is present", coachExportHtml.includes("Adapter database exp&eacute;rimental") && coachExportHtml.includes("database-adapter-spike-section"), "database adapter spike section visible"),
    check("experimental database spike guard is visible", coachExportHtml.includes("aucune base r&eacute;elle n&rsquo;est lue ou &eacute;crite") && fullMatchWorkbenchChainReplay5EValidation.includes("real database write count is 0"), "no-real-DB guard visible"),
    check("experimental database adapter appendix is present", coachExportHtml.includes("D&eacute;tails adapter database exp&eacute;rimental") && coachExportHtml.includes("active product history source file_backed"), "database adapter spike appendix visible"),
    check("real database read and write counts are 0", fullMatchWorkbenchChainReplay5EValidation.includes("real database write count is 0") && fullMatchWorkbenchChainReplay5EValidation.includes("real database read count is 0") && coachExportHtml.includes("real DB read count: 0"), "real DB counters 0"),
    check("adapter implemented but not production ready", fullMatchWorkbenchChainReplay5E.includes("adapter implemented: true") && fullMatchWorkbenchChainReplay5E.includes("adapter production ready: false"), "experimental adapter only"),
    check("feature flag disabled by default", fullMatchWorkbenchChainReplay5EValidation.includes("feature flag is present and disabled by default") && fullMatchWorkbenchChainReplay5E.includes("default feature flag enabled: false"), "feature flag guard visible"),
    check("product database activation remains disabled", fullMatchWorkbenchChainReplay5E.includes("product activation allowed: false") && fullMatchWorkbenchChainReplay5E.includes("database used as product truth: false"), "product DB activation disabled"),
    check("save-result scenarios pass", fullMatchWorkbenchChainReplay5EValidation.includes("inserted save scenario passes") && fullMatchWorkbenchChainReplay5EValidation.includes("replaced save scenario passes") && fullMatchWorkbenchChainReplay5EValidation.includes("ignored duplicate save scenario passes"), "save-result scenarios visible"),
    check("report queries remain dry-run", fullMatchWorkbenchChainReplay5EValidation.includes("query by team passes") && fullMatchWorkbenchChainReplay5EValidation.includes("query by phase passes"), "dry-run query guard visible"),
    check("bundle includes database adapter spike source files", bundleReports.includes("src/reports/history/databaseHistoryAdapterFeatureFlag.ts") && bundleReports.includes("src/reports/history/experimentalDatabaseCoachMatchHistoryAdapter.ts") && bundleReports.includes("src/reports/coachReportDatabaseAdapterSpike.ts") && bundleReports.includes("src/reports/buildCoachReportDatabaseAdapterSpike.ts"), "5E source bundled"),
    check("bundle includes database adapter spike tests", bundleReports.includes("databaseHistoryAdapterFeatureFlag.test.ts") && bundleReports.includes("experimentalDatabaseCoachMatchHistoryAdapter.test.ts") && bundleReports.includes("coachReportDatabaseAdapterSpikeRenderer.test.ts") && bundleReports.includes("coachReportDatabaseAdapterSpikeGuard.test.ts"), "5E tests bundled"),
    check("simulation bundle includes scoring guard 5E", bundleSimulation.includes("scoringGuard.5e.test.ts"), "5E scoring guard bundled"),
    check("database adapter spike evidence category bundled", bundleContracts.includes("WORKBENCH_CHAIN_COACH_REPORT_DATABASE_ADAPTER_SPIKE") && bundleSimulation.includes("WORKBENCH_CHAIN_COACH_REPORT_DATABASE_ADAPTER_SPIKE"), "5E evidence category bundled"),
    check("main export hides internal status names", !containsAny(coachExportMainHtml, ["officially_confirmed", "trace_supported", "sandbox_only"]), "internal statuses hidden"),
    check("main export avoids recommendation wording", !containsAny(coachExportMainHtml, ["meilleur choix", "composition recommand", "selection automatique", "preuve globale", "certitude"]), "recommendation wording count 0"),
    check("main export avoids selection wording", !containsAny(coachExportMainHtml, ["a selectionner", "player selected"]), "selection wording count 0"),
    check("visible French copy is clean", !containsAny(coachExportHtml, coachHtmlMojibakeMarkers), "mojibake count 0"),
    check("no scoring constants changed", scoringEvents.includes("SHOT_GOAL") && scoringEvents.includes("TRY_TOUCHDOWN") && scoringEvents.includes("PENALTY_SHOT") && fullMatchWorkbenchChainReplay5EValidation.includes("scoring constants unchanged"), "scoring constants visible"),
    check("no MatchBonusEvent mutation", scoringEvents.includes("MatchBonusEvent") && scoringEvents.includes("not part of this live ScoringEvent stream") && fullMatchWorkbenchChainReplay5EValidation.includes("MatchBonusEvent unchanged"), "MatchBonusEvent separated"),
    check("batch/live separation preserved", scoringEvents.includes("batch/live separation status: PASS") && fullMatchWorkbenchChainReplay5EValidation.includes("FULL_MATCH_BATCH_ECONOMY remains the only global economy proof"), "batch/live PASS"),
    check("50-match economy remains global reference", fullMatchWorkbenchChainReplay5E.includes("FULL_MATCH_BATCH_ECONOMY remains the only global economy proof") && bundleSimulation.includes("VALIDATED_FULL_MATCH_ECONOMY_ANCHOR"), "50-match reference visible"),
    check("explicit exhaustive test command available", readIfExists(join(shareDirectory, "package.json")).includes("\"test:all\"") && fullMatchWorkbenchChainReplay5EValidation.includes("explicit exhaustive test command is available"), "test:all visible"),
    check("recommendations visible", fullMatchWorkbenchChainReplay5EValidation.includes("CONFIRM_EXPERIMENTAL_DATABASE_ADAPTER_SPIKE") && fullMatchWorkbenchChainReplay5EValidation.includes("CONFIRM_NO_PRODUCT_DATABASE_ACTIVATION") && fullMatchWorkbenchChainReplay5EValidation.includes("PREPARE_DATABASE_ADAPTER_IMPLEMENTATION_OR_UI_WIRING"), "5E recommendations visible"),
  ];
  ...
  const sprint4WChecks: readonly SharePackCheck[] = [
    ...
    check("detailed manifest exists", detailedManifest.length > 0, detailedManifestPath),
    check("validation.share-pack.md copied", sourceExists("validation.share-pack.md") && requiredCopied("validation.share-pack.md"), "validation.share-pack.md"),
    check("all expected files are copied", sprint4WExpectedFiles.every((file) => requiredCopied(file)), sprint4WExpectedFiles.filter((file) => !requiredCopied(file)).join(", ") || "all copied"),
    check("all expected files are listed in manifest", sprint4WExpectedFiles.every((file) => manifest.includes(file)), sprint4WExpectedFiles.filter((file) => !manifest.includes(file)).join(", ") || "all listed"),
    check("no stale files remain in reports/share", staleFiles.length === 0, staleFiles.join(", ") || "none"),
    check("excluded-by-default files are not in reports/share", excludedInShare.length === 0, excludedInShare.join(", ") || "none"),
    check("source reports were not deleted", missingExcludedSources.length === 0, missingExcludedSources.join(", ") || "0"),
    check("manifest exposes MINIMAL_REVIEW", manifest.includes("MINIMAL_REVIEW"), "mode visible"),
    check("manifest says upload every file in reports/share", manifest.includes("Upload every file in this reports/share directory."), "upload instruction visible"),
    check("current sprint is Sprint 4W", activeConfig.sprintName === "Sprint 4W - Phase Visual Readability & Legend Calibration", activeConfig.sprintName),
    check("share pack mode is MINIMAL_REVIEW", activeConfig.mode === "MINIMAL_REVIEW", activeConfig.mode),
    check("share pack under 20 files", filesOnDisk.length <= 20, String(filesOnDisk.length)),
    check("expected share file count is 20", filesOnDisk.length === 20, String(filesOnDisk.length)),
    check("missing expected files are none", sprint4WExpectedFiles.every((file) => requiredCopied(file)), sprint4WExpectedFiles.filter((file) => !requiredCopied(file)).join(", ") || "none"),
    check("previous sprint leftovers are 0", sprint4WForbiddenLeftovers.every((file) => !requiredCopied(file)), sprint4WForbiddenLeftovers.filter((file) => requiredCopied(file)).join(", ") || "0"),
    check("README is Sprint 4W oriented", readme.includes("# Sprint 4W Share Pack") && readme.includes("fullmatch-workbench-chain-replay-4w.md") && readme.includes("coach-report.export.html"), "README current"),
    check("4W report included", fullMatchWorkbenchChainReplay4W.includes("# FullMatch Workbench Chain Replay 4W") && fullMatchWorkbenchChainReplay4W.includes("Phase Visual Readability"), "4W doc included"),
    check("4W validation is PASS", fullMatchWorkbenchChainReplay4WValidation.includes("Status: PASS") && fullMatchWorkbenchChainReplay4WValidation.includes("Phase Visual Readability status is available"), "4W validation current"),
    check("product report HTML copied", coachProductHtml.includes("Rapport coach") && coachProductHtml.includes("Joueurs"), "product HTML visible"),
    check("export report HTML copied", coachExportHtml.includes("Rapport coach") && coachExportHtml.includes("data-export-snapshot=\"coach_product_report\""), "export HTML visible"),
    check("phase legend is present", coachExportHtml.includes("L&eacute;gende des cartes terrain") && coachExportHtml.includes("phase-pitch-legend"), "legend visible"),
    check("phase pitch panels are present", coachExportHtml.includes("phase-pitch-grid") && coachExportHtml.includes("Avec ballon") && coachExportHtml.includes("Sans ballon") && coachExportHtml.includes("Dernier rempart"), "phase visuals visible"),
    check("primary and secondary emphasis are present", coachExportHtml.includes("phase-zone--primary") && coachExportHtml.includes("phase-zone--secondary"), "zone emphasis visible"),
    check("readability guard remains visible", coachExportHtml.includes("La couleur et l'intensit&eacute; des zones servent") && fullMatchWorkbenchChainReplay4WValidation.includes("visible copy avoids recommendation wording"), "readability guard visible"),
    check("premium export uses single source of truth", coachExportHtml.includes("Ce rapport export&eacute; reprend la lecture du rapport produit. Il ne cr&eacute;e pas une seconde source de v&eacute;rit&eacute;.") && fullMatchWorkbenchChainReplay4WValidation.includes("single source of truth"), "single source visible"),
    check("main export hides internal status names", !containsAny(coachExportMainHtml, ["officially_confirmed", "trace_supported", "sandbox_only"]), "internal statuses hidden"),
    check("main export avoids recommendation wording", !containsAny(coachExportMainHtml, ["meilleur choix", "composition recommand", "selection automatique"]), "recommendation wording count 0"),
    check("main export avoids selection wording", !containsAny(coachExportMainHtml, ["a selectionner", "player selected"]), "selection wording count 0"),
    check("visible French copy is clean", !containsAny(coachExportHtml, coachHtmlMojibakeMarkers), "mojibake count 0"),
    check("bundle includes phase-visual readability source files", bundleReports.includes("src/reports/coachReportPhaseVisualReadability.ts") && bundleReports.includes("src/reports/buildCoachReportPhaseVisualReadability.ts") && bundleReports.includes("src/reports/renderTacticalPitchPanel.ts"), "4W source bundled"),
    check("bundle includes phase-visual readability tests", bundleReports.includes("coachReportPhaseVisualReadability.test.ts") && bundleReports.includes("phaseVisualLegendRenderer.test.ts") && bundleReports.includes("tacticalPitchPanelReadabilityRenderer.test.ts") && bundleReports.includes("coachReportPhaseVisualReadabilityExportRenderer.test.ts") && bundleReports.includes("coachReportPhaseVisualReadabilitySourceGuard.test.ts") && bundleReports.includes("coachReportPhaseVisualReadabilityCopy.test.ts") && bundleReports.includes("coachReportPhaseVisualReadabilityPrintCss.test.ts") && bundleReports.includes("coachReportPhaseVisualReadabilityGuard.test.ts"), "4W tests bundled"),
    check("simulation bundle includes scoring guard 4W", bundleSimulation.includes("scoringGuard.4w.test.ts") && bundleSimulation.includes("validateScoringGuard4W"), "4W scoring guard bundled"),
    check("readability cannot mutate score", coachExportHtml.includes("score mutation count 0") || fullMatchWorkbenchChainReplay4WValidation.includes("readability layer cannot mutate official score"), "score mutation forbidden"),
    check("readability cannot mutate possession", coachExportHtml.includes("possession mutation count 0") || fullMatchWorkbenchChainReplay4WValidation.includes("readability layer cannot mutate official possession"), "possession mutation forbidden"),
    check("readability cannot create production scoring events", coachExportHtml.includes("production scoring event creation count 0") || fullMatchWorkbenchChainReplay4WValidation.includes("readability layer cannot create production scoring events"), "production scoring creation forbidden"),
    check("readability cannot claim global economy", fullMatchWorkbenchChainReplay4WValidation.includes("readability layer cannot claim global economy"), "global economy forbidden"),
    check("no scoring constants changed", scoringEvents.includes("SHOT_GOAL") && scoringEvents.includes("TRY_TOUCHDOWN") && scoringEvents.includes("PENALTY_SHOT") && fullMatchWorkbenchChainReplay4WValidation.includes("scoring constants unchanged"), "scoring constants visible"),
    check("no MatchBonusEvent mutation", scoringEvents.includes("MatchBonusEvent") && scoringEvents.includes("not part of this live ScoringEvent stream") && fullMatchWorkbenchChainReplay4WValidation.includes("MatchBonusEvent unchanged"), "MatchBonusEvent separated"),
    check("batch/live separation preserved", scoringEvents.includes("batch/live separation status: PASS") && fullMatchWorkbenchChainReplay4WValidation.includes("FULL_MATCH_BATCH_ECONOMY remains the only global economy proof"), "batch/live PASS"),
    check("50-match economy remains global reference", fullMatchWorkbenchChainReplay4W.includes("FULL_MATCH_BATCH_ECONOMY remains the only global economy proof") && bundleSimulation.includes("VALIDATED_FULL_MATCH_ECONOMY_ANCHOR"), "50-match reference visible"),
    check("explicit exhaustive test command available", readIfExists(join(shareDirectory, "package.json")).includes("\"test:all\"") && fullMatchWorkbenchChainReplay4WValidation.includes("explicit exhaustive test command is available"), "test:all visible"),
    check("recommendations visible", fullMatchWorkbenchChainReplay4WValidation.includes("CONFIRM_PHASE_VISUAL_READABILITY") && fullMatchWorkbenchChainReplay4WValidation.includes("CONFIRM_LEGEND_AND_ZONE_HIERARCHY"), "4W recommendations visible"),
  ];
  const sprint4VChecks: readonly SharePackCheck[] = [
    check("reports/share exists", existsSync(shareDirectory), shareDirectory),
    check("manifest exists", manifest.length > 0, manifestPath),
    check("README exists", readme.length > 0, readmePath),
    check("detailed manifest exists", detailedManifest.length > 0, detailedManifestPath),
    check("validation.share-pack.md copied", sourceExists("validation.share-pack.md") && requiredCopied("validation.share-pack.md"), "validation.share-pack.md"),
    check("all expected files are copied", sprint4VExpectedFiles.every((file) => requiredCopied(file)), sprint4VExpectedFiles.filter((file) => !requiredCopied(file)).join(", ") || "all copied"),
    check("all expected files are listed in manifest", sprint4VExpectedFiles.every((file) => manifest.includes(file)), sprint4VExpectedFiles.filter((file) => !manifest.includes(file)).join(", ") || "all listed"),
    check("no stale files remain in reports/share", staleFiles.length === 0, staleFiles.join(", ") || "none"),
    check("excluded-by-default files are not in reports/share", excludedInShare.length === 0, excludedInShare.join(", ") || "none"),
    check("source reports were not deleted", missingExcludedSources.length === 0, missingExcludedSources.join(", ") || "0"),
    check("manifest exposes MINIMAL_REVIEW", manifest.includes("MINIMAL_REVIEW"), "mode visible"),
    check("manifest says upload every file in reports/share", manifest.includes("Upload every file in this reports/share directory."), "upload instruction visible"),
    check("current sprint is Sprint 4V", activeConfig.sprintName === "Sprint 4V - Phase Visuals & Tactical Pitch Panels", activeConfig.sprintName),
    check("share pack mode is MINIMAL_REVIEW", activeConfig.mode === "MINIMAL_REVIEW", activeConfig.mode),
    check("share pack under 20 files", filesOnDisk.length <= 20, String(filesOnDisk.length)),
    check("expected share file count is 20", filesOnDisk.length === 20, String(filesOnDisk.length)),
    check("missing expected files are none", sprint4VExpectedFiles.every((file) => requiredCopied(file)), sprint4VExpectedFiles.filter((file) => !requiredCopied(file)).join(", ") || "none"),
    check("previous sprint leftovers are 0", sprint4VForbiddenLeftovers.every((file) => !requiredCopied(file)), sprint4VForbiddenLeftovers.filter((file) => requiredCopied(file)).join(", ") || "0"),
    check("README is Sprint 4V oriented", readme.includes("# Sprint 4V Share Pack") && readme.includes("fullmatch-workbench-chain-replay-4v.md") && readme.includes("coach-report.export.html"), "README current"),
    check("4V report included", fullMatchWorkbenchChainReplay4V.includes("# FullMatch Workbench Chain Replay 4V") && fullMatchWorkbenchChainReplay4V.includes("Phase Visuals"), "4V doc included"),
    check("4V validation is PASS", fullMatchWorkbenchChainReplay4VValidation.includes("Status: PASS") && fullMatchWorkbenchChainReplay4VValidation.includes("three phase panels exist"), "4V validation current"),
    check("product report HTML copied", coachProductHtml.includes("Rapport coach") && coachProductHtml.includes("Joueurs"), "product HTML visible"),
    check("export report HTML copied", coachExportHtml.includes("Rapport coach") && coachExportHtml.includes("data-export-snapshot=\"coach_product_report\""), "export HTML visible"),
    check("phase pitch panels are present", coachExportHtml.includes("phase-pitch-grid") && coachExportHtml.includes("Avec ballon") && coachExportHtml.includes("Sans ballon") && coachExportHtml.includes("Dernier rempart"), "phase visuals visible"),
    check("phase-visual guard remains visible", coachExportHtml.includes("Les cartes terrain affichent uniquement les signaux") && fullMatchWorkbenchChainReplay4VValidation.includes("visible recommendation wording count is 0"), "guard visible"),
    check("premium export uses single source of truth", coachExportHtml.includes("Ce rapport export&eacute; reprend la lecture du rapport produit. Il ne cr&eacute;e pas une seconde source de v&eacute;rit&eacute;.") && fullMatchWorkbenchChainReplay4VValidation.includes("product/export score matches"), "single source visible"),
    check("main export hides internal status names", !containsAny(coachExportMainHtml, ["officially_confirmed", "trace_supported", "sandbox_only"]), "internal statuses hidden"),
    check("main export avoids recommendation wording", !containsAny(coachExportMainHtml, ["meilleur choix", "composition recommand", "selection automatique"]), "recommendation wording count 0"),
    check("main export avoids selection wording", !containsAny(coachExportMainHtml, ["a selectionner", "player selected"]), "selection wording count 0"),
    check("visible French copy is clean", !containsAny(coachExportHtml, coachHtmlMojibakeMarkers), "mojibake count 0"),
    check("bundle includes phase-visual source files", bundleReports.includes("src/reports/coachReportPhaseVisuals.ts") && bundleReports.includes("src/reports/buildCoachReportPhaseVisuals.ts") && bundleReports.includes("src/reports/renderTacticalPitchPanel.ts"), "4V source bundled"),
    check("bundle includes phase-visual tests", bundleReports.includes("coachReportPhaseVisuals.test.ts") && bundleReports.includes("tacticalPitchPanelRenderer.test.ts") && bundleReports.includes("coachReportPhaseVisualsExportRenderer.test.ts") && bundleReports.includes("coachReportPhaseVisualsSourceGuard.test.ts") && bundleReports.includes("coachReportPhaseVisualsCopy.test.ts") && bundleReports.includes("coachReportPhaseVisualsPrintCss.test.ts") && bundleReports.includes("coachReportPhaseVisualsGuard.test.ts"), "4V tests bundled"),
    check("simulation bundle includes scoring guard 4V", bundleSimulation.includes("scoringGuard.4v.test.ts") && bundleSimulation.includes("validateScoringGuard4V"), "4V scoring guard bundled"),
    check("phase visuals cannot mutate score", coachExportHtml.includes("score mutation count 0") || fullMatchWorkbenchChainReplay4VValidation.includes("phase visuals cannot mutate official score"), "score mutation forbidden"),
    check("phase visuals cannot mutate possession", coachExportHtml.includes("possession mutation count 0") || fullMatchWorkbenchChainReplay4VValidation.includes("phase visuals cannot mutate official possession"), "possession mutation forbidden"),
    check("phase visuals cannot create production scoring events", coachExportHtml.includes("production scoring event creation count 0") || fullMatchWorkbenchChainReplay4VValidation.includes("phase visuals cannot create production scoring events"), "production scoring creation forbidden"),
    check("phase visuals cannot claim global economy", fullMatchWorkbenchChainReplay4VValidation.includes("phase visuals cannot claim global economy"), "global economy forbidden"),
    check("no scoring constants changed", scoringEvents.includes("SHOT_GOAL") && scoringEvents.includes("TRY_TOUCHDOWN") && scoringEvents.includes("PENALTY_SHOT") && fullMatchWorkbenchChainReplay4VValidation.includes("scoring constants unchanged"), "scoring constants visible"),
    check("no MatchBonusEvent mutation", scoringEvents.includes("MatchBonusEvent") && scoringEvents.includes("not part of this live ScoringEvent stream") && fullMatchWorkbenchChainReplay4VValidation.includes("MatchBonusEvent unchanged"), "MatchBonusEvent separated"),
    check("batch/live separation preserved", scoringEvents.includes("batch/live separation status: PASS") && fullMatchWorkbenchChainReplay4VValidation.includes("FULL_MATCH_BATCH_ECONOMY remains the only global economy proof"), "batch/live PASS"),
    check("50-match economy remains global reference", fullMatchWorkbenchChainReplay4V.includes("FULL_MATCH_BATCH_ECONOMY remains the only global economy proof") && bundleSimulation.includes("VALIDATED_FULL_MATCH_ECONOMY_ANCHOR"), "50-match reference visible"),
    check("explicit exhaustive test command available", readIfExists(join(shareDirectory, "package.json")).includes("\"test:all\"") && fullMatchWorkbenchChainReplay4VValidation.includes("explicit exhaustive test command is available"), "test:all visible"),
    check("recommendations visible", fullMatchWorkbenchChainReplay4VValidation.includes("CONFIRM_PHASE_VISUALS_FROM_PRODUCT_SOURCE") && fullMatchWorkbenchChainReplay4VValidation.includes("CONFIRM_TACTICAL_PITCH_PANELS_STAY_NON_MUTATING"), "4V recommendations visible"),
  ];
  ...
  const sprint4TChecks: readonly SharePackCheck[] = [
    ...
    check("detailed manifest exists", detailedManifest.length > 0, detailedManifestPath),
    check("validation.share-pack.md copied", sourceExists("validation.share-pack.md") && requiredCopied("validation.share-pack.md"), "validation.share-pack.md"),
    check("all expected files are copied", sprint4TExpectedFiles.every((file) => requiredCopied(file)), sprint4TExpectedFiles.filter((file) => !requiredCopied(file)).join(", ") || "all copied"),
    check("all expected files are listed in manifest", sprint4TExpectedFiles.every((file) => manifest.includes(file)), sprint4TExpectedFiles.filter((file) => !manifest.includes(file)).join(", ") || "all listed"),
    check("no stale files remain in reports/share", staleFiles.length === 0, staleFiles.join(", ") || "none"),
    check("excluded-by-default files are not in reports/share", excludedInShare.length === 0, excludedInShare.join(", ") || "none"),
    check("source reports were not deleted", missingExcludedSources.length === 0, missingExcludedSources.join(", ") || "0"),
    check("manifest exposes MINIMAL_REVIEW", manifest.includes("MINIMAL_REVIEW"), "mode visible"),
    check("manifest says upload every file in reports/share", manifest.includes("Upload every file in this reports/share directory."), "upload instruction visible"),
    check("current sprint is Sprint 4T", activeConfig.sprintName === "Sprint 4T - Coach Report PDF Export & Share Snapshot", activeConfig.sprintName),
    check("share pack mode is MINIMAL_REVIEW", activeConfig.mode === "MINIMAL_REVIEW", activeConfig.mode),
    check("share pack under 20 files", filesOnDisk.length <= 20, String(filesOnDisk.length)),
    check("expected share file count is 20", filesOnDisk.length === 20, String(filesOnDisk.length)),
    check("missing expected files are none", sprint4TExpectedFiles.every((file) => requiredCopied(file)), sprint4TExpectedFiles.filter((file) => !requiredCopied(file)).join(", ") || "none"),
    check("previous sprint leftovers are 0", sprint4TForbiddenLeftovers.every((file) => !requiredCopied(file)), sprint4TForbiddenLeftovers.filter((file) => requiredCopied(file)).join(", ") || "0"),
    check("README is Sprint 4T oriented", readme.includes("# Sprint 4T Share Pack") && readme.includes("fullmatch-workbench-chain-replay-4t.md") && readme.includes("coach-report.export.html"), "README current"),
    check("4T report included", fullMatchWorkbenchChainReplay4T.includes("# FullMatch Workbench Chain Replay 4T") && fullMatchWorkbenchChainReplay4T.includes("Coach Report Export Snapshot"), "4T doc included"),
    check("4T validation is PASS", fullMatchWorkbenchChainReplay4TValidation.includes("Status: PASS") && fullMatchWorkbenchChainReplay4TValidation.includes("coach-report.export.html is generated"), "4T validation current"),
    check("product report HTML copied", coachProductHtml.includes("Rapport coach") && coachProductHtml.includes("Joueurs"), "product HTML visible"),
    check("export report HTML copied", coachExportHtml.includes("Rapport coach") && coachExportHtml.includes("data-export-snapshot=\"coach_product_report\""), "export HTML visible"),
    check("export uses single source of truth", coachExportHtml.includes("single source of truth: true") && fullMatchWorkbenchChainReplay4TValidation.includes("single source of truth"), "single source visible"),
    check("duplicated report logic is false", coachExportHtml.includes("duplicated report logic: false") && fullMatchWorkbenchChainReplay4TValidation.includes("duplicated report logic is false"), "duplicate logic false"),
    check("product/export section counts match", fullMatchWorkbenchChainReplay4T.includes("product/export section counts match: YES.") || fullMatchWorkbenchChainReplay4TValidation.includes("product/export section counts match."), "section count match visible"),
    check("product/export scores match", fullMatchWorkbenchChainReplay4T.includes("product/export scores match: YES.") || fullMatchWorkbenchChainReplay4TValidation.includes("product/export scores match."), "score match visible"),
    check("candidate comparison matches product", fullMatchWorkbenchChainReplay4T.includes("candidate comparison matches product: YES.") || fullMatchWorkbenchChainReplay4TValidation.includes("candidate comparison matches"), "comparison match visible"),
    check("interpretation guard remains visible", coachExportHtml.includes("Les rapprochements profil-joueur ne sont pas des choix de composition.") && fullMatchWorkbenchChainReplay4TValidation.includes("interpretation guard remains visible"), "guard visible"),
    check("print CSS is present", coachExportHtml.includes("@media print") && coachExportHtml.includes("@page"), "print CSS visible"),
    check("page-break CSS is present", coachExportHtml.includes("break-inside: avoid") && coachExportHtml.includes("page-break-inside: avoid"), "page break CSS visible"),
    check("export cards avoid page breaks", coachExportHtml.includes(".product-card") && coachExportHtml.includes(".comparison-card") && coachExportHtml.includes(".appendix"), "break guards visible"),
    check("main export hides internal status names", !containsAny(coachExportMainHtml, ["officially_confirmed", "trace_supported", "sandbox_only"]), "internal statuses hidden"),
    check("main export avoids recommendation wording", !containsAny(coachExportMainHtml, ["meilleur choix", "joueur recommandÃ©", "recommandÃ©", "titulaire conseillÃ©", "remplacement conseillÃ©", "composition recommandÃ©e", "sÃ©lection automatique", "selection automatique"]), "recommendation wording count 0"),
    check("main export avoids selection wording", !containsAny(coachExportMainHtml, ["a sÃ©lectionner", "a selectionner", "joueur sÃ©lectionnÃ©", "player selected"]), "selection wording count 0"),
    check("visible French copy is clean", !containsAny(coachExportHtml, coachHtmlMojibakeMarkers), "mojibake count 0"),
    check("export appendix contains snapshot tags", coachExportHtml.includes("product HTML path: reports/coach-report.product.html") && coachExportHtml.includes("export HTML path: reports/coach-report.export.html") && coachExportHtml.includes("production scoring event creation count: 0"), "appendix visible"),
    check("export snapshot evidence category bundled", bundleContracts.includes("WORKBENCH_CHAIN_COACH_REPORT_EXPORT_SNAPSHOT") && bundleSimulation.includes("WORKBENCH_CHAIN_COACH_REPORT_EXPORT_SNAPSHOT"), "4T evidence category bundled"),
    check("export model, builder, and renderer bundled", bundleReports.includes("src/reports/coachReportExportSnapshot.ts") && bundleReports.includes("src/reports/buildCoachReportExportSnapshot.ts") && bundleReports.includes("src/reports/renderCoachReportExportHtml.ts"), "4T files bundled"),
    check("export tests bundled", bundleReports.includes("coachReportExportSnapshot.test.ts") && bundleReports.includes("coachReportExportHtml.test.ts") && bundleReports.includes("coachReportExportPrintCss.test.ts") && bundleReports.includes("coachReportExportCopy.test.ts") && bundleReports.includes("coachReportExportGuard.test.ts"), "4T report tests bundled"),
    check("scoring guard 4T bundled", bundleSimulation.includes("scoringGuard.4t.test.ts") && bundleSimulation.includes("validateScoringGuard4T"), "4T scoring guard bundled"),
    check("no player is selected", coachExportHtml.includes("player selected count: 0") && fullMatchWorkbenchChainReplay4TValidation.includes("no player is selected"), "non-applied"),
    check("no automatic selection is true", coachExportHtml.includes("automatic selection count: 0") && fullMatchWorkbenchChainReplay4TValidation.includes("no automatic selection is true"), "auto-selection forbidden"),
    check("export cannot mutate score", coachExportHtml.includes("score mutation count: 0") && fullMatchWorkbenchChainReplay4TValidation.includes("cannot mutate official score"), "score mutation forbidden"),
    check("export cannot mutate possession", coachExportHtml.includes("possession mutation count: 0") && fullMatchWorkbenchChainReplay4TValidation.includes("cannot mutate official possession"), "possession mutation forbidden"),
    check("export cannot create production scoring events", coachExportHtml.includes("production scoring event creation count: 0") && fullMatchWorkbenchChainReplay4TValidation.includes("cannot create production scoring events"), "production scoring creation forbidden"),
    check("export cannot claim global economy", coachExportHtml.includes("global economy claim count: 0") && fullMatchWorkbenchChainReplay4TValidation.includes("cannot claim global economy"), "global economy forbidden"),
    check("no scoring constants changed", scoringEvents.includes("SHOT_GOAL") && scoringEvents.includes("TRY_TOUCHDOWN") && scoringEvents.includes("PENALTY_SHOT") && fullMatchWorkbenchChainReplay4TValidation.includes("scoring constants unchanged"), "scoring constants visible"),
    check("no MatchBonusEvent mutation", scoringEvents.includes("MatchBonusEvent") && scoringEvents.includes("not part of this live ScoringEvent stream") && fullMatchWorkbenchChainReplay4TValidation.includes("MatchBonusEvent unchanged"), "MatchBonusEvent separated"),
    check("batch/live separation preserved", scoringEvents.includes("batch/live separation status: PASS") && fullMatchWorkbenchChainReplay4TValidation.includes("FULL_MATCH_BATCH_ECONOMY remains the only global economy proof"), "batch/live PASS"),
    check("50-match economy remains global reference", fullMatchWorkbenchChainReplay4T.includes("FULL_MATCH_BATCH_ECONOMY remains the only global economy proof") && bundleSimulation.includes("VALIDATED_FULL_MATCH_ECONOMY_ANCHOR"), "50-match reference visible"),
    check("explicit exhaustive test command available", readIfExists(join(shareDirectory, "package.json")).includes("\"test:all\"") && fullMatchWorkbenchChainReplay4TValidation.includes("explicit exhaustive test command is available"), "test:all visible"),
    check("recommendations visible", fullMatchWorkbenchChainReplay4TValidation.includes("CONFIRM_COACH_REPORT_EXPORT_SNAPSHOT") && fullMatchWorkbenchChainReplay4TValidation.includes("CONFIRM_PRINT_READY_EXPORT"), "4T recommendations visible"),
  ];
  const sprint4SChecks: readonly SharePackCheck[] = [
    check("reports/share exists", existsSync(shareDirectory), shareDirectory),
    check("manifest exists", manifest.length > 0, manifestPath),
    check("README exists", readme.length > 0, readmePath),
    check("detailed manifest exists", detailedManifest.length > 0, detailedManifestPath),
    check("validation.share-pack.md copied", sourceExists("validation.share-pack.md") && requiredCopied("validation.share-pack.md"), "validation.share-pack.md"),
    check("all expected files are copied", allRequiredCopied, missingExpectedFiles.join(", ") || "all copied"),
    check("all expected files are listed in manifest", allRequiredListed, allowlistedFiles.filter((file) => !manifest.includes(file)).join(", ") || "all listed"),
    check("no stale files remain in reports/share", staleFiles.length === 0, staleFiles.join(", ") || "none"),
    check("excluded-by-default files are not in reports/share", excludedInShare.length === 0, excludedInShare.join(", ") || "none"),
    check("source reports were not deleted", missingExcludedSources.length === 0, missingExcludedSources.join(", ") || "0"),
    check("manifest exposes MINIMAL_REVIEW", manifest.includes("MINIMAL_REVIEW"), "mode visible"),
    check("manifest says upload every file in reports/share", manifest.includes("Upload every file in this reports/share directory."), "upload instruction visible"),
    check("current sprint is Sprint 4S", activeConfig.sprintName === "Sprint 4S - Player Card Polish & Candidate Comparison", activeConfig.sprintName),
    check("share pack mode is MINIMAL_REVIEW", activeConfig.mode === "MINIMAL_REVIEW", activeConfig.mode),
    check("share pack under 20 files", filesOnDisk.length <= 20, String(filesOnDisk.length)),
    check("expected share file count is 19", filesOnDisk.length === 19, String(filesOnDisk.length)),
    check("missing expected files are none", sprint4SExpectedFiles.every((file) => requiredCopied(file)), sprint4SExpectedFiles.filter((file) => !requiredCopied(file)).join(", ") || "none"),
    check("previous sprint leftovers are 0", sprint4SForbiddenLeftovers.every((file) => !requiredCopied(file)), sprint4SForbiddenLeftovers.filter((file) => requiredCopied(file)).join(", ") || "0"),
    check("all required current sprint files copied", sprint4SExpectedFiles.every((file) => requiredCopied(file)), sprint4SExpectedFiles.filter((file) => !requiredCopied(file)).join(", ") || "all copied"),
    check("manifest lists Sprint 4S", manifest.includes("Sprint 4S - Player Card Polish & Candidate Comparison") && detailedManifest.includes("Sprint 4S - Player Card Polish & Candidate Comparison"), "visible"),
    check("README is Sprint 4S oriented", readme.includes("# Sprint 4S Share Pack") && readme.includes("fullmatch-workbench-chain-replay-4s.md"), "README current"),
    check("4S report included", fullMatchWorkbenchChainReplay4S.includes("# FullMatch Workbench Chain Replay 4S") && fullMatchWorkbenchChainReplay4S.includes("Player Candidate Comparison View"), "4S doc included"),
    check("4S validation is PASS", fullMatchWorkbenchChainReplay4SValidation.includes("Status: PASS") && fullMatchWorkbenchChainReplay4SValidation.includes("Player Candidate Comparison View status is available"), "4S validation PASS"),
    check("product report HTML copied", coachProductHtml.includes("Rapport coach") && coachProductHtml.includes("Joueurs"), "product HTML visible"),
    check("comparison evidence category bundled", bundleContracts.includes("WORKBENCH_CHAIN_PLAYER_CANDIDATE_COMPARISON_VIEW") && bundleSimulation.includes("WORKBENCH_CHAIN_PLAYER_CANDIDATE_COMPARISON_VIEW"), "4S evidence category bundled"),
    check("comparison model, builder, and roster coverage inputs bundled", bundleReports.includes("src/reports/playerCandidateComparisonView.ts") && bundleReports.includes("src/reports/buildPlayerCandidateComparisonView.ts") && bundleReports.includes("src/reports/rosterCoverageMatchup.ts"), "4S files bundled"),
    check("comparison tests bundled", bundleReports.includes("playerCandidateComparisonView.test.ts") && bundleReports.includes("playerCandidateComparisonRenderer.test.ts") && bundleReports.includes("playerCandidateComparisonCopy.test.ts") && bundleReports.includes("playerCandidateComparisonDensity.test.ts") && bundleReports.includes("playerCandidateComparisonGuard.test.ts"), "4S report tests bundled"),
    check("scoring guard 4S bundled", bundleSimulation.includes("scoringGuard.4s.test.ts") && bundleSimulation.includes("validateScoringGuard4S"), "4S scoring guard bundled"),
    ...
    check("comparison guard sentence visible", coachProductHtml.includes("Les cartes comparent des pistes d'observation") || coachProductHtml.includes("Les cartes comparent des pistes dâ€™observation"), "visible comparison explanation"),
    check("comparison appendix present", coachProductHtml.includes("Details de comparaison des candidats") && coachProductHtml.includes("compact visible candidate count") && coachProductHtml.includes("<details class=\"appendix\">"), "appendix present"),
    ...
    check("main product report avoids lineup recommendation wording", !containsAny(coachProductMainHtml, ["best", "recommended", "meilleur choix", "composition recommand", "titulaire", "remplacement", "selection automatique", "sÃƒÆ’Ã‚Â©lection automatique"]), "forbidden selection wording count 0"),
    check("visible French copy is clean", !containsAny(coachProductHtml, coachHtmlMojibakeMarkers), "mojibake count 0"),
    check("comparison tags are emitted in appendices", coachProductHtml.includes("player_candidate_comparison_view_status_available") && coachProductHtml.includes("player_candidate_comparison_max_compact_candidates_per_profile_3") && coachProductHtml.includes("player_candidate_comparison_player_selected_count_0"), "4S tags emitted"),
    check("comparison priority labels are visible", coachProductHtml.includes("A etudier en priorite") && coachProductHtml.includes("Alternative a comparer"), "priority labels visible"),
    check("compact density cap is visible through validation", fullMatchWorkbenchChainReplay4SValidation.includes("compact candidates per profile are capped at 3") && fullMatchWorkbenchChainReplay4SValidation.includes("extra candidates are collapsed by default"), "density guard visible"),
    check("no automatic selection visible", fullMatchWorkbenchChainReplay4SValidation.includes("no automatic selection is true") && (fullMatchWorkbenchChainReplay4SValidation.includes("player selected count: 0") || fullMatchWorkbenchChainReplay4SValidation.includes("player selected count is 0")), "non-applied"),
    check("comparison cannot mutate score", coachProductHtml.includes("coach_product_report_score_mutation_count_0") && fullMatchWorkbenchChainReplay4SValidation.includes("comparison view cannot mutate official score"), "score mutation forbidden"),
    check("comparison cannot mutate possession", coachProductHtml.includes("possession mutation count: 0") && fullMatchWorkbenchChainReplay4SValidation.includes("comparison view cannot mutate official possession"), "possession mutation forbidden"),
    check("comparison cannot create production scoring events", coachProductHtml.includes("production scoring event creation count: 0") && fullMatchWorkbenchChainReplay4SValidation.includes("comparison view cannot create production scoring events"), "production scoring creation forbidden"),
    check("comparison cannot claim global economy", coachProductHtml.includes("global economy claim count: 0") && fullMatchWorkbenchChainReplay4SValidation.includes("comparison view cannot claim global economy"), "global economy forbidden"),
    check("no scoring constants changed", scoringEvents.includes("SHOT_GOAL") && scoringEvents.includes("TRY_TOUCHDOWN") && scoringEvents.includes("PENALTY_SHOT") && fullMatchWorkbenchChainReplay4SValidation.includes("scoring constants unchanged"), "scoring constants visible"),
    check("no MatchBonusEvent mutation", scoringEvents.includes("MatchBonusEvent") && scoringEvents.includes("not part of this live ScoringEvent stream") && fullMatchWorkbenchChainReplay4SValidation.includes("MatchBonusEvent unchanged"), "MatchBonusEvent separated"),
    check("batch/live separation preserved", scoringEvents.includes("batch/live separation status: PASS") && fullMatchWorkbenchChainReplay4SValidation.includes("FULL_MATCH_BATCH_ECONOMY remains the only global economy proof"), "batch/live PASS"),
    check("50-match economy remains global reference", fullMatchWorkbenchChainReplay4S.includes("FULL_MATCH_BATCH_ECONOMY remains the only global economy proof") && bundleSimulation.includes("VALIDATED_FULL_MATCH_ECONOMY_ANCHOR"), "50-match reference visible"),
    check("explicit exhaustive test command available", readIfExists(join(shareDirectory, "package.json")).includes("\"test:all\"") && fullMatchWorkbenchChainReplay4SValidation.includes("explicit exhaustive test command is available"), "test:all visible"),
    check("recommendations visible", fullMatchWorkbenchChainReplay4SValidation.includes("CONFIRM_PLAYER_CANDIDATE_COMPARISON_VIEW") && fullMatchWorkbenchChainReplay4SValidation.includes("CONFIRM_CANDIDATE_SECTION_READABILITY"), "4S recommendations visible"),
  ];
  ...
  const checks = activeConfig.sprintName.includes("Role Fit UI Implementation")
    ...
    : activeConfig.sprintName.includes("Sprint 5F - Durable Storage Decision & Disabled Real Adapter Wiring")
      ? sprint5FChecks
    : activeConfig.sprintName.includes("Sprint 5E - Database Adapter Implementation Spike Without Product Activation")
      ? sprint5EChecks
    ...
    : activeConfig.sprintName.includes("Sprint 4W - Phase Visual Readability & Legend Calibration")
      ? sprint4WChecks
    : activeConfig.sprintName.includes("Sprint 4V - Phase Visuals & Tactical Pitch Panels")
      ? sprint4VChecks
