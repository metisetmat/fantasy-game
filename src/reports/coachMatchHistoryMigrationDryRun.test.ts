...
export function validateCoachMatchHistoryMigrationDryRun(): readonly string[] {
  ...
  const dryRun = buildCoachMatchHistoryMigrationDryRun({
    sourceRecords: [record, record, invalidRecord, unsupportedRecord],
  ...
  assertTest(dryRun.sourceRecordCount === 4, "source record count is present.");
  assertTest(dryRun.migrationPlanCount === 4, "migration plan count is present.");
  ...
  assertTest(dryRun.rejectedInvalidCount === 1, "rejected invalid records are handled safely.");
