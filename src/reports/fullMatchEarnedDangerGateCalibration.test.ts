import {
  buildFullMatchEarnedDangerGateCalibrationModel,
...
function event(input: {
  ...
  return {
    ...
    matchId: "reset-break-blowout-6N-test",
    ...
    tacticalContext: {
      ...
      reason: "Earned Danger Gate regression fixture",
    },
    fatigueContext: {
      teamCondition: 72,
...
function reportWithTimeline(timeline: readonly MatchEvent[]): MatchReport {
  return {
    ...
    reportMeta: {
      ...
      generatorVersion: "reset-break-blowout-6N-test",
      generatedFrom: "runFullMatch",
      sourceOfTruthNote: "Earned Danger Gate regression fixture",
      limitations: [],
    },
    timeline,
    teamStats: [
...
const model = buildFullMatchEarnedDangerGateCalibrationModel();
const doc = renderFullMatchEarnedDangerGate6NDoc(model);
const validation = renderFullMatchEarnedDangerGate6NValidation(model);

assertTest(model.scope === "FULL_MATCH_EARNED_DANGER_GATE_CALIBRATION", "6N model scope must be official.");
assertTest(model.version === "EARNED_DANGER_GATE_6N", "6N model version must be stable.");
assertTest(model.matchCount >= 50, "6N calibration must run at least 50 matches.");
assertTest(model.blowoutAudits.length >= 50, "6N blowout audits must exist.");
assertTest(model.resetToDangerQualityAudits.length >= 50, "6N reset-to-danger quality audits must exist.");
assertTest(model.scoreFromScoreChangeAllRuns, "6N official score must come from score_change consequences.");
assertTest(model.officialPathConnectedAllRuns, "6N official path must stay connected.");
assertTest(!model.scoreCapApplied, "6N must not apply score caps.");
assertTest(!model.postHocRewriteApplied, "6N must not rewrite scores.");
assertTest(!model.scoringEventsDeleted, "6N must not delete scoring events.");
assertTest(!model.forcedOpponentScoreApplied, "6N must not force opponent scores.");
assertTest(!model.forcedTrailingTeamScoreApplied, "6N must not force trailing-team scores.");
assertTest(!model.MatchBonusEventChanged, "6N must not mutate MatchBonusEvent.");
assertTest(model.batchLiveSeparationPreserved, "6N batch/live separation must remain preserved.");
assertTest(!model.persistenceUsedForScoring && !model.sqliteUsedForScoring, "6N persistence and SQLite must not score.");
assertTest(model.unknownScoringFamilyCount === 0, "6N UNKNOWN scoring family must not leak.");
assertTest(model.penaltyShotActiveLeakageCount === 0, "6N PENALTY_SHOT must not leak.");
assertTest(doc.includes("Blowout Root Cause Audit Summary"), "6N doc must include root cause table.");
assertTest(doc.includes("Reset-To-Danger Quality Audit Summary"), "6N doc must include reset-to-danger table.");
assertTest(validation.includes("Explicit Exhaustive Test Command"), "6N validation must include exhaustive command.");
...
console.log("fullMatchEarnedDangerGateCalibration tests passed.");
