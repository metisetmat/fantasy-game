# Coach Action Plan Cards & Training Focus Packaging 7C

## Summary
- status: PASS
- scope: COACH_ACTION_PLAN_CARDS_TRAINING_FOCUS_PACKAGING
- version: COACH_ACTION_PLAN_CARDS_TRAINING_FOCUS_PACKAGING_7C
- baselineVersion: COACH_INSIGHT_DEPTH_NEXT_MATCH_RECOMMENDATIONS_7B
- matchEconomyBaselinePreserved: true
- productReportReady: true
- coachExportReady: true
- sourceOfTruthSeparationPreserved: true
- actionPlanCardsReady: true
- trainingFocusPackagingReady: true
- nextMatchPlanPackaged: true
- coachLanguagePolished: true
- mechanicalWordingRemoved: true
- productBaselineReady: true
- recommendation: KEEP_COACH_ACTION_PLAN_PACKAGING
- nextSprintRecommendation: 7D - Coach Report Premium Layout & Visual Hierarchy

## Baseline Preservation
| Baseline | Value |
| --- | --- |
| 7B status | PARTIAL |
| 7B productBaselineReady | false |
| 7A status | PARTIAL |
| 6X averageTotalPoints | 22.2 |
| 6X closeGameRate | 50% |
| guardrailsPreserved | true |

## Action Plan Cards
### Securiser la premiere sortie apres recuperation
- priority: primary
- sourceType: official
- confidence: medium
- observation: Les recuperations existent, mais leur valeur depend de la premiere passe ou du premier relais.
- coachingProblem: La premiere sortie doit rester propre quand la pression revient vite.
- trainingFocus: Offrir une solution proche au porteur apres recuperation.
- coachAction: Observer la qualite du premier relais avant d'en faire une consigne collective.
- nextMatchObservableSignal: La premiere sortie reste propre sous pression.
- successIndicator: Moins de pertes immediates apres recuperation.
- riskOrTradeoff: Trop de soutien proche peut ralentir la projection ou exposer la rest-defense.
- estimatedReadTimeSeconds: 21
### Transformer les zones de danger en continuite
- priority: secondary
- sourceType: official
- confidence: medium
- observation: Certaines zones de danger se repetent et demandent une deuxieme action controlee.
- coachingProblem: Le danger doit devenir une sequence, pas seulement une entree isolee.
- trainingFocus: Enchainer apres l'entree en zone dangereuse plutot que finir trop vite.
- coachAction: Verifier si la progression cree un soutien actif apres le premier gain.
- nextMatchObservableSignal: La progression mene a une deuxieme action controlee.
- successIndicator: Plus de situations avec soutien actif apres progression.
- riskOrTradeoff: Chercher la continuite peut ralentir l'action ou perdre la profondeur.
- estimatedReadTimeSeconds: 21
### Garder une structure apres pression ou arret du gardien
- priority: watch
- sourceType: official
- confidence: low
- observation: La fin de sequence doit rester organisee apres pression, tir, arret ou rebond.
- coachingProblem: Une action neutralisee ne doit pas ouvrir une recuperation adverse propre.
- trainingFocus: Surveiller le second ballon, le rebond et la rest-defense.
- coachAction: Regarder si l'equipe reste placee quand la premiere action est neutralisee.
- nextMatchObservableSignal: L'equipe garde une structure lisible apres pression ou arret.
- successIndicator: Moins de recuperations adverses propres apres action dangereuse.
- riskOrTradeoff: Sur-engagement, fatigue et rebonds centraux peuvent fragiliser le bloc.
- estimatedReadTimeSeconds: 21

## Training Focus Packaging
| Metric | Value |
| --- | --- |
| trainingFocusCount | 2 |
| primaryTrainingFocusCount | 1 |
| trainingFocusWithWhyCount | 2 |
| trainingFocusWithCoachCueCount | 2 |
| trainingFocusWithObservableSignalCount | 2 |
| genericDrillCueCount | 0 |

## Next Match Plan Packaging
| Metric | Value |
| --- | --- |
| nextMatchPriorityCount | 3 |
| concretePriorityCount | 3 |
| vaguePriorityCount | 0 |
| priorityWithObservableSignalCount | 3 |
| priorityWithSuccessIndicatorCount | 3 |
| planReadingTimeEstimate | 27 |

## Wording Polish Audit
| Metric | Value |
| --- | --- |
| duplicatedLabelCount | 0 |
| mechanicalPhraseCount | 0 |
| awkwardSentenceCount | 0 |
| repeatedPrefixCount | 0 |
| jargonCount | 0 |
| forbiddenWordingCount | 0 |
| unresolvedTemplatePlaceholderCount | 0 |

## Warning Codes
- ACTION_PLAN_CARDS_VISIBLE
- ACTION_PLAN_CARDS_CONCRETE
- SUCCESS_INDICATORS_VISIBLE
- OBSERVABLE_SIGNALS_VISIBLE
- TRADEOFFS_VISIBLE
- TRAINING_FOCUS_PACKAGED
- NEXT_MATCH_PLAN_PACKAGED
- COACH_LANGUAGE_POLISHED
- MECHANICAL_WORDING_REMOVED
- OFFICIAL_DIAGNOSTIC_SANDBOX_SEPARATION_PRESERVED
- PRODUCT_REPORT_READY
- COACH_EXPORT_READY
- MATCH_ECONOMY_BASELINE_PRESERVED